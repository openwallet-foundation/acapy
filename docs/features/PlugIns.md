...
Example log output:
```
INFO: Auto-installing plugins from acapy-plugins repository: webvh, connection_update (current ACA-Py version (1.7.0))
INFO: Installing plugin: webvh (version: 1.7.0)
INFO: Successfully installed plugin: webvh (version: 1.7.0)
```
