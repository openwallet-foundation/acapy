...
async def test_shutdown_with_active_tasks(
    ...
    """Test shutdown cancels active background tasks."""
    processor_called = asyncio.Event()
    processor_can_finish = asyncio.Event()

    async def slow_processor(profile, event):
        """A processor that waits to be explicitly allowed to finish."""
        processor_called.set()
        await processor_can_finish.wait()

    event_bus.subscribe(re.compile(".*"), slow_processor)

    with patch.object(test_module.LOGGER, "debug") as mock_debug:
        # Start the event processing (creates background task)
        await event_bus.notify(profile, event)

        # Wait for processor to start
        await processor_called.wait()

        # Verify we have an active task
    ...
    assert any(
        "Shutting down EventBus" in str(call) for call in mock_debug.call_args_list
    )
    assert any(
        "EventBus shutdown complete" in str(call) for call in mock_debug.call_args_list
    )


@pytest.mark.asyncio
async def test_shutdown_timeout_warning(
    event_bus: EventBus, profile: MagicMock, event: Event
):
    """Test shutdown logs warning when tasks don't cancel within timeout."""

    async def uncancellable_processor(profile, event):
        """A processor that ignores cancellation."""
        try:
    ...
    event_bus.subscribe(re.compile(".*"), uncancellable_processor)

    # Mock asyncio.wait to simulate timeout
    with (
        patch("asyncio.wait", side_effect=asyncio.TimeoutError),
        patch.object(test_module.LOGGER, "warning") as mock_warning,
        patch.object(test_module.LOGGER, "debug"),
    ):
        await event_bus.notify(profile, event)
        ...
        assert "did not cancel within timeout" in mock_warning.call_args[0][0]
