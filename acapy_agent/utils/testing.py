...
def remove_test_profile_wallet(profile: Optional[AskarProfile] = None):
    """Remove the wallet associated with the profile."""
    if profile:
        shutil.rmtree(profile.store.uri.replace("sqlite://", "").replace("sqlite.db", ""))
