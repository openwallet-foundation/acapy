...
class KanonAnonCredsProfile(Profile):
    ...
    async def remove(self):
        ...
        if errors:
            raise ProfileError("Errors occurred while removing profile: " + "; ".join(errors))
    ...
    def bind_providers(self):
        ...
        injector.bind_provider(
            BaseStorageSearch,
            ClassProvider("acapy_agent.storage.kanon_storage.KanonStorageSearch", ref(self)),
        )
        injector.bind_provider(
            VCHolder,
            ClassProvider(
        ...
        injector.bind_provider(
            IndyIssuer,
            ClassProvider("acapy_agent.indy.credx.issuer_kanon.KanonIndyCredxIssuer", ref(self)),
        )
        if (
...
class KanonAnonCredsProfileSession(ProfileSession):
    ...
    def __init__(
        ...
        *,
        ...
        if is_txn:
            ...
            self._askar_opener = profile.opened.askar_store.transaction(profile.profile_id)
        else:
    ...
    def is_transaction(self) -> bool:
        if self._dbstore_handle and self._askar_handle:
            return self._dbstore_handle.is_transaction and self._askar_handle.is_transaction
        if self._dbstore_opener and self._askar_opener:
            return self._dbstore_opener.is_transaction and self._askar_opener.is_transaction
...
class KanonAnonProfileManager(ProfileManager):
    ...
    async def provision(
        ...
        opened = await store_config.open_store(provision=True, in_memory=config.get("test"))
    ...
    async def open(
        ...
        store_config = KanonStoreConfig(config)  # No store_class specialization needed
        opened = await store_config.open_store(provision=False, in_memory=config.get("test"))
