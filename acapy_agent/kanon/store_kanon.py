...
from ..database_manager.dbstore import DBStoreError, DBStoreErrorCode, DBStoreSession, DBStore
...
class KanonStoreConfig:
    ...
    def __init__(self, config: Optional[dict] = None, store_class: str = "dbstore"):
        ...
        self.key_derivation_method = config.get("key_derivation_method") or self.DEFAULT_KEY_DERIVATION
        self.rekey = config.get("rekey")
        self.rekey_derivation_method = config.get("rekey_derivation_method") or self.DEFAULT_KEY_DERIVATION
            ...
            try:
                ...
                for key in required_keys:
                    if key not in config_dict:
                        raise ProfileError(f"Missing required key '{key}' in dbstore_storage_config")
                if "tls" in config_dict and isinstance(config_dict["tls"], dict):
                    tls_config = config_dict["tls"]
                    if "sslmode" in tls_config and tls_config["sslmode"] not in ["disable", "allow", "prefer", "require", "verify-ca", "verify-full"]:
                        raise ProfileError("Invalid sslmode in tls configuration")
            except json.JSONDecodeError as e:
                LOGGER.error("Invalid JSON in dbstore_storage_config: %s", self.dbstore_storage_config)
        ...
        self.dbstore_schema_config = config.get("dbstore_schema_config", self.DEFAULT_SCHEMA_CONFIG)
        ...
        if dbstore_storage_type not in ("postgres", "sqlite"):
            raise ProfileError(f"Unsupported dbstore storage type: {dbstore_storage_type}")
        ...
        if self.dbstore_storage_type == "postgres" and self.dbstore_storage_creds:
            try:
                ...
                LOGGER.error("Invalid JSON in dbstore_storage_creds: %s", self.dbstore_storage_creds)
    ...
    def get_dbstore_uri(self, create: bool = False, in_memory: Optional[bool] = False) -> str:
        LOGGER.debug("Generating DBStore URI with dbstore_storage_type=%s, create=%s, in_memory=%s", 
                    self.dbstore_storage_type, create, in_memory)
        ...
        if self.dbstore_storage_type == "sqlite":
            ...
            db_name = urllib.parse.quote(self.name + "_dbstore")  # add _dbstore in the db name to ensure no conflict with askar
    ...
    def get_askar_uri(self, create: bool = False, in_memory: Optional[bool] = False) -> str:
    ...
    async def remove_store(self):
        try:
            if self.store_class == "askar":
                ...
                config = json.loads(self.dbstore_storage_config) if self.dbstore_storage_config else {}
            ...
            raise ProfileError(f"Error removing {self.store_class} store: {str(err)}") from err
    ...
    async def open_store(self, provision: bool = False, in_memory: Optional[bool] = False) -> "KanonOpenStore":
        ...
        try:
            if provision:
                release_number = "release_0" if self.dbstore_schema_config == "generic" else self.CURRENT_SCHEMA_RELEASE_NUMBER
        ...
        try:
            if provision:
                askar_store = await Store.provision(
                    askar_uri, self.key_derivation_method, self.key, profile=self.name, recreate=self.auto_recreate
                )
            else:
                askar_store = await Store.open(askar_uri, self.key_derivation_method, self.key, profile=self.name)
        except AskarError as err:
            self._handle_askar_open_error(err, retry=True)
            if self.rekey:
                try:
                    askar_store = await Store.open(askar_uri, self.key_derivation_method, self.DEFAULT_KEY, profile=self.name)
...
class KanonOpenStore:
    def __init__(self, config: KanonStoreConfig, created, db_store: DBStore, askar_store: Store):
        self.config = config
