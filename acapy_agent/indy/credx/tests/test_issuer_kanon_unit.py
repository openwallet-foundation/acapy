...
class _Handle:
    ...
    async def insert(self, cat, name, value=None, tags=None, value_json=None):
        if value is None and value_json is not None:
            self.rows[(cat, name)] = types.SimpleNamespace(raw_value=None, value_json=value_json, tags=tags or {})
        else:
            self.rows[(cat, name)] = types.SimpleNamespace(raw_value=value, value_json=value if isinstance(value, dict) else None, tags=tags or {})
    async def replace(self, cat, name, value=None, tags=None, expiry_ms=None, value_json=None):
...
def patched_issuer(monkeypatch):
    ...
    class _CredDef:
        ...
        @staticmethod
        def create(*a, **k):
            return _CredDef(), types.SimpleNamespace(to_json_buffer=lambda: b"{}"), types.SimpleNamespace(to_json_buffer=lambda: b"{}")
        def to_json(self):
            return "{}"
        @staticmethod
        def load(raw):
    ...
    class _RevRegDef:
        ...
        @staticmethod
        def create(*a, **k):
            return _RevRegDef(), types.SimpleNamespace(to_json_buffer=lambda: b"{}"), types.SimpleNamespace(to_json=lambda: "{}"), types.SimpleNamespace(to_json=lambda: "{}")
        @staticmethod
        def load(raw):
            return _RevRegDef()
...
async def test_create_schema_and_cred_def_and_offer(patched_issuer):
    ...
    class _TestIssuer(m.KanonIndyCredxIssuer):
        async def merge_revocation_registry_deltas(self, fro_delta: str, to_delta: str) -> str:
            return "{}"
        async def create_and_store_revocation_registry(self, origin_did: str, cred_def_id: str, revoc_def_type: str, tag: str, max_cred_num: int, tails_base_path: str):
            return ("revreg", "{}", "{}")
    issuer = _TestIssuer(prof)
    sid, sjson = await issuer.create_schema("did:sov:abc", "s", "1.0", ["name"])
    assert sid == "sch"
    cdid, cdjson = await issuer.create_and_store_credential_definition("did:sov:abc", {"id": "sch"})
    assert cdid == "cd"
...
@pytest.mark.asyncio
async def test_save_revocation_updates_missing_and_concurrent(patched_issuer, monkeypatch):
    m = patched_issuer
    prof = _Profile()
    class _TestIssuer(m.KanonIndyCredxIssuer):
        async def merge_revocation_registry_deltas(self, fro_delta: str, to_delta: str) -> str:
    ...
    await issuer2._save_revocation_updates("rr", types.SimpleNamespace(to_json_buffer=lambda: b"{}"), {"curr_id": 1}, set())
            ...
            class _H:
                async def fetch(self, cat, name, for_update=False):
                    if str(cat).endswith("info"):
                        return types.SimpleNamespace(value_json={"curr_id": 2, "used_ids": []})
    ...
    with pytest.raises(m.IndyIssuerRetryableError):
        await issuer3._save_revocation_updates("rr", types.SimpleNamespace(to_json_buffer=lambda: b"{}"), {"curr_id": 1, "used_ids": []}, {1})
...
@pytest.mark.asyncio
async def test_create_cred_def_store_error_and_offer_create_error(patched_issuer, monkeypatch):
    m = patched_issuer
    prof = _Profile()
    class _TestIssuer(m.KanonIndyCredxIssuer):
        async def merge_revocation_registry_deltas(self, fro_delta: str, to_delta: str) -> str:
    ...
    class _OkCredDef:
        ...
        @staticmethod
        def create(*a, **k):
            return _OkCredDef(), types.SimpleNamespace(to_json_buffer=lambda: b"{}"), types.SimpleNamespace(to_json_buffer=lambda: b"{}")
        def to_json(self):
            return "{}"
        @staticmethod
        def load(raw):
...
async def test_create_credential_revocation_full_and_success(patched_issuer, monkeypatch):
    ...
    async with prof.transaction() as t:
        await t.handle.insert(m.CATEGORY_REV_REG, "rr", types.SimpleNamespace(to_json_buffer=lambda: b"{}"))
        await t.handle.insert(m.CATEGORY_REV_REG_INFO, "rr", {"curr_id": 5, "used_ids": []})
    ...
    async with prof.transaction() as t:
        await t.handle.insert(m.CATEGORY_REV_REG_INFO, "rr", {"curr_id": 0, "used_ids": []})
...
def test_parse_revocation_components_load_errors(patched_issuer, monkeypatch):
    ...
    def _raise_load(raw):
        raise m.CredxError(1, "x")
    monkeypatch.setattr(m, "RevocationRegistryDefinition", types.SimpleNamespace(load=_raise_load))
    with pytest.raises(m.IndyIssuerError):
...
async def test_save_revocation_updates_success_path(patched_issuer):
    ...
    async with prof.transaction() as t:
        await t.handle.insert(m.CATEGORY_REV_REG, "rr", types.SimpleNamespace(to_json_buffer=lambda: b"{}"))
        await t.handle.insert(m.CATEGORY_REV_REG_INFO, "rr", {"curr_id": 1, "used_ids": []})
    class _Sess(_Txn):
        async def fetch(self, cat, name, for_update=False):
            key = (cat, name)
    ...
    class _P(_Profile):
        def transaction(self):
            return _Sess(self.h)
    await issuer._save_revocation_updates("rr", types.SimpleNamespace(to_json_buffer=lambda: b"{}"), {"curr_id": 1, "used_ids": []}, {2})
...
async def test_create_credential_revocation_success_path(patched_issuer, monkeypatch):
    ...
    class _RRD:
        max_cred_num = 10
    monkeypatch.setattr(m, "RevocationRegistryDefinition", types.SimpleNamespace(load=lambda raw: _RRD()))
    monkeypatch.setattr(issuer, "_update_revocation_registry", lambda comps, ids: types.SimpleNamespace(to_json=lambda: "{}"))
...
@pytest.mark.asyncio
async def test_retryable_class_methods_merge_and_create_rev_reg(patched_issuer, monkeypatch):
    m = patched_issuer
    class _Delta:
        @staticmethod
        def load(x):
    ...
    err._profile = prof
    out = await err.create_and_store_revocation_registry("did:sov:abc", "cd", "CL_ACCUM", "tag", 5, "/tmp")
