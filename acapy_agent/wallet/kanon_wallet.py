...
class KanonWallet(BaseWallet):
    ...
    async def create_signing_key(
        ...
        LOGGER.debug("Entering create_signing_key with key_type: %s, seed: %s, metadata: %s", key_type, seed, metadata)
    ...
    async def create_key(
        ...
        LOGGER.debug("Entering create_key with key_type: %s, seed: %s, metadata: %s, kid: %s", key_type, seed, metadata, kid)
        if metadata is None:
            metadata = {}
        ...
        try:
            ...
            if err.code == AskarErrorCode.DUPLICATE:
                raise WalletDuplicateError("Verification key already present in wallet") from None
    ...
    async def assign_kid_to_key(self, verkey: str, kid: str) -> KeyInfo:
        ...
        try:
            ...
            key_entry = await self._session.askar_handle.fetch_key(name=verkey, for_update=True)
            if not key_entry:
    ...
    async def replace_signing_key_metadata(self, verkey: str, metadata: dict):
        ...
        LOGGER.debug("Entering replace_signing_key_metadata with verkey: %s, metadata: %s", verkey, metadata)
        if not verkey:
            LOGGER.error("No verkey provided")
            raise WalletNotFoundError("No key identifier provided")
        ...
        try:
            LOGGER.debug("Fetching key entry for verkey: %s", verkey)
            key_entry = await self._session.askar_handle.fetch_key(verkey, for_update=True)
            if not key_entry:
    ...
    async def create_local_did(
        ...
        LOGGER.debug("Entering create_local_did with method: %s, key_type: %s, seed: %s, did: %s, metadata: %s", method, key_type, seed, did, metadata)
        ...
        async with self._session.store.session() as session:
            try:
                ...
                if item:
                    ...
                    LOGGER.debug("Inserting new DID with value: %s, tags: %s", value_json, tags)
        ...
        result = DIDInfo(did=did, verkey=verkey, metadata=metadata, method=method, key_type=key_type)
    ...
    async def replace_local_did_metadata(self, did: str, metadata: dict):
        ...
        LOGGER.debug("Entering replace_local_did_metadata with did: %s, metadata: %s", did, metadata)
        async with self._session.store.session() as session:
            try:
                LOGGER.debug("Fetching DID entry for: %s", did)
                item = await session.fetch(CATEGORY_DID, did, for_update=True)
    ...
    async def set_did_endpoint(
        ...
        LOGGER.debug("Entering set_did_endpoint with did: %s, endpoint: %s", did, endpoint)
        ...
        if (
            ...
            if not ledger.read_only:
                ...
                async with ledger:
                    ...
                    if not write_ledger:
                        LOGGER.debug("set_did_endpoint returning attrib_def: %s", attrib_def)
                        return attrib_def
    ...
    async def rotate_did_keypair_start(
        ...
        LOGGER.debug("Entering rotate_did_keypair_start with did: %s, next_seed: %s", did, next_seed)
        did_methods = self._session.inject(DIDMethods)
        did_method = did_methods.from_did(did)
        if not did_method.supports_rotation:
            LOGGER.error("DID method %s does not support rotation", did_method.method_name)
            raise WalletError(f"DID method '{did_method.method_name}' does not support key rotation")
        ...
        try:
            ...
            if err.code != AskarErrorCode.DUPLICATE:
                raise WalletError("Error when creating new keypair for local DID") from err
            LOGGER.debug("Key already exists, proceeding")
    ...
    async def verify_message(
        ...
        LOGGER.debug("Entering verify_message with from_verkey: %s, key_type: %s", from_verkey, key_type)
        if not from_verkey:
            LOGGER.error("Verkey not provided")
            raise WalletError("Verkey not provided")
        if not signature:
    ...
    async def pack_message(
        ...
        LOGGER.debug("Entering pack_message with to_verkeys: %s, from_verkey: %s", to_verkeys, from_verkey)
        if message is None:
            LOGGER.error("Message not provided")
            raise WalletError("Message not provided")
        try:
    ...
    async def unpack_message(self, enc_message: bytes) -> Tuple[str, str, str]:
        ...
        try:
            ...
            unpacked_json, recipient, sender = await unpack_message(self._session.askar_handle, enc_message)
    ...
    def _load_did_entry(self, entry: Entry) -> DIDInfo:
        ...
        result = DIDInfo(
            ...
            key_type=key_types.from_key_type(did_info.get("verkey_type", "ed25519")) or ED25519,
        )
