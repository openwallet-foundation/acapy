...
async def test_assert_no_callenge_error(manager: VcDiManager):
    with pytest.raises(VcDiManagerError):
        await manager.verify_presentation({}, {"options": {}})


@pytest.mark.asyncio
async def test_assert_verify_presentation(manager: VcDiManager, profile: Profile):
    anoncreds_registry = mock.MagicMock(AnonCredsRegistry, autospec=True)
    anoncreds_registry.get_schema = mock.CoroutineMock(
    ...
    anoncreds_registry.get_credential_definition = mock.CoroutineMock(
        return_value=mock.MagicMock(
    ...
    profile.context.injector.bind_instance(AnonCredsRegistry, anoncreds_registry)
