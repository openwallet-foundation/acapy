...
from .base import DEFAULT_PAGE_SIZE, BaseStorage, BaseStorageSearch, BaseStorageSearchSession, validate_record
from .error import StorageDuplicateError, StorageError, StorageNotFoundError, StorageSearchError
...
class KanonStorage(BaseStorage):
    ...
    async def add_record(self, record: StorageRecord, session: Optional[DBStoreSession] = None):
    ...
    async def _add_record(self, record: StorageRecord, session: DBStoreSession):
        try:
            ...
            if err.code == DBStoreErrorCode.DUPLICATE:
                raise StorageDuplicateError(f"Duplicate record: {record.type}/{record.id}") from None
            raise StorageError("Error when adding storage record") from err

    async def get_record(self, record_type: str, record_id: str, options: Optional[Mapping] = None, session: Optional[DBStoreSession] = None) -> StorageRecord:
        ...
        if session is None:
            async with self._session.store.session() as temp_session:
                return await self._get_record(record_type, record_id, for_update, temp_session)
        return await self._get_record(record_type, record_id, for_update, session)

    async def _get_record(self, record_type: str, record_id: str, for_update: bool, session: DBStoreSession) -> StorageRecord:
        try:
    ...
    async def update_record(self, record: StorageRecord, value: str, tags: Mapping, session: Optional[DBStoreSession] = None):
    ...
    async def _update_record(self, record: StorageRecord, value: str, tags: Mapping, session: DBStoreSession):
        try:
            ...
            if err.code == DBStoreErrorCode.NOT_FOUND:
                raise StorageNotFoundError(f"Record not found: {record.type}/{record.id}") from None
            raise StorageError("Error when updating storage record value") from err

    async def delete_record(self, record: StorageRecord, session: Optional[DBStoreSession] = None):
    ...
    async def find_record(self, type_filter: str, tag_query: Mapping, options: Optional[Mapping] = None, session: Optional[DBStoreSession] = None) -> StorageRecord:
        """Find a single record matching the type and tag query."""
        for_update = bool(options and options.get("forUpdate"))
        if session is None:
            async with self._session.store.session() as temp_session:
                return await self._find_record(type_filter, tag_query, for_update, temp_session)
        return await self._find_record(type_filter, tag_query, for_update, session)

    async def _find_record(self, type_filter: str, tag_query: Mapping, for_update: bool, session: DBStoreSession) -> StorageRecord:
        try:
            results = await session.fetch_all(type_filter, tag_query, limit=2, for_update=for_update)
        except DBStoreError as err:
    ...
    async def find_paginated_records(
        ...
        LOGGER.debug(f"KanonStorage.find_paginated_records called with type_filter={type_filter}, tag_query={tag_query}, limit={limit}, offset={offset}, order_by={order_by}, descending={descending}")
        results = []
    ...
    async def find_paginated_records_keyset(
        ...
        LOGGER.debug(f"KanonStorage.find_paginated_records_keyset called with type_filter={type_filter}, tag_query={tag_query}, last_id={last_id}, limit={limit}, order_by={order_by}, descending={descending}")
        results = []
    ...
    async def find_all_records(
        ...
        if session is None:
            async with self._session.store.session() as temp_session:
                return await self._find_all_records(type_filter, tag_query, order_by, descending, for_update, temp_session)
        return await self._find_all_records(type_filter, tag_query, order_by, descending, for_update, session)
    ...
    async def delete_all_records(self, type_filter: str, tag_query: Optional[Mapping] = None, session: Optional[DBStoreSession] = None):
    ...
    async def _delete_all_records(self, type_filter: str, tag_query: Optional[Mapping], session: DBStoreSession):
        try:
...
class KanonStorageSearch(BaseStorageSearch):
    ...
    def search_records(self, type_filter: str, tag_query: Optional[Mapping] = None, page_size: Optional[int] = None, options: Optional[Mapping] = None) -> "KanonStorageSearchSession":
        return KanonStorageSearchSession(self._profile, type_filter, tag_query, page_size, options)

class KanonStorageSearchSession(BaseStorageSearchSession):
    def __init__(self, profile, type_filter: str, tag_query: Mapping, page_size: Optional[int] = None, options: Optional[Mapping] = None):
    ...
    async def fetch(self, max_count: Optional[int] = None, offset: Optional[int] = None) -> Sequence[StorageRecord]:
        if self._done:
            raise StorageSearchError("Search query is complete")
    ...
    async def _open(self, offset: Optional[int] = None, limit: Optional[int] = None):
        if self._scan:
            return
        try:
            LOGGER.debug("Opening scan for type_filter=%s, tag_query=%s, limit=%s, offset=%s", 
                        self.type_filter, self.tag_query, limit, offset)
