...
conn_str = "postgres://myuser:mypass@192.168.2.172:5432/test_scan_normalize?sslmode=prefer"
...
PRES_JSON = {
    "presentation": {
        "identities": ["cred_id_123"],
        "proof": {"proof_type": "Ed25519"}
    }
}
...
async def setup_data(store: DBStore, num_records: int = 50):
    ...
    for i in range(num_records):
        async with store.transaction() as session:
            ...
            LOGGER.debug(f"[setup_data] Attempting to insert record {name} with expiry_ms={expiry_ms}")
    ...
    async with store.session() as session:
        ...
        LOGGER.debug(f"[setup_data] Inserted {count} non-expired records: {inserted_names}")
        expected_count = num_records - 5  # Expect 5 expired records to be filtered out
        assert count == expected_count, f"Expected {expected_count} non-expired records, got {count}"
        assert len(inserted_names) == num_records, f"Expected {num_records} total insertions, got {len(inserted_names)}"
...
async def test_scan_with_filter(store: DBStore):
    ...
    LOGGER.debug(f"[test_scan_with_filter] Found {len(entries)} records: {[entry.name for entry in entries]}")
    assert len(entries) == expected_count, f"Expected {expected_count} active records, got {len(entries)}"
    for entry in entries:
        assert json.loads(entry.value)["state"] == "active", f"Entry {entry.name} should have state=active"
...
async def test_scan_with_complex_filter(store: DBStore):
    ...
    scan = store.scan(category="pres_ex_v20", tag_filter=complex_tag_filter, profile=profile_name)
    entries = [entry async for entry in scan]
    ...
    for entry in entries[:5]:
        print(f" - {entry.name}: {json.loads(entry.value)}")
        LOGGER.debug(f"[test_scan_with_complex_filter] Entry {entry.name}: {json.loads(entry.value)}")
    assert len(entries) == expected_count, f"Expected {expected_count} records, got {len(entries)}"
    for entry in entries:
        value = json.loads(entry.value)
        assert value["state"] == "active" or (value["state"] == "pending" and value["verified"] == "true"), f"Entry {entry.name} does not match filter"
...
async def test_scan_paginated(store: DBStore):
    ...
    limit = 5
    offset = 10
    scan = store.scan(category="pres_ex_v20", tag_filter=tag_filter, limit=limit, offset=offset, profile=profile_name)
    entries = [entry async for entry in scan]
...
async def test_scan_sorted(store: DBStore):
    ...
    scan = store.scan(category="pres_ex_v20", profile=profile_name, order_by="thread_id", descending=False)
    entries = [entry async for entry in scan]
    ...
    scan = store.scan(category="pres_ex_v20", profile=profile_name, order_by="state", descending=True)
    entries = [entry async for entry in scan]
    ...
    assert states == sorted(states, reverse=True), "Entries not sorted by state descending"
...
async def test_scan_invalid_order_by(store: DBStore):
    ...
    try:
        scan = store.scan(category="pres_ex_v20", profile=profile_name, order_by="invalid_column")
        async for _ in scan:
            pass
        ...
        assert "Invalid order_by column" in str(e), "Expected DatabaseError for invalid order_by"
...
async def test_scan_keyset_basic(store: DBStore):
    ...
    scan = store.scan_keyset(category="pres_ex_v20", last_id=first_id, limit=10, profile=profile_name)
    entries = [entry async for entry in scan]
    ...
    for i, entry in enumerate(entries[1:], 1):
        assert json.loads(entry.value)["thread_id"] > json.loads(entries[i-1].value)["thread_id"], "Entries not in order"
...
async def test_scan_keyset_with_filter(store: DBStore):
    ...
    async with store.session() as session:
        entries = await session.fetch_all(category="pres_ex_v20", tag_filter=tag_filter, limit=1)
        assert len(entries) == 1, "Expected 1 pending entry to get last_id"
        first_id = (await session.count(category="pres_ex_v20")) - len(entries) + 1

    scan = store.scan_keyset(category="pres_ex_v20", tag_filter=tag_filter, last_id=first_id, limit=5, profile=profile_name)
    entries = [entry async for entry in scan]
    expected_count = 5
    ...
    assert len(entries) <= expected_count, f"Expected up to {expected_count} records, got {len(entries)}"
    for entry in entries:
        assert json.loads(entry.value)["state"] == "pending", f"Entry {entry.name} should have state=pending"
...
async def test_scan_keyset_sorted(store: DBStore):
    ...
    scan = store.scan_keyset(category="pres_ex_v20", last_id=first_id, limit=5, order_by="connection_id", descending=False, profile=profile_name)
    entries = [entry async for entry in scan]
    ...
    scan = store.scan_keyset(category="pres_ex_v20", last_id=first_id, limit=5, order_by="connection_id", descending=True, profile=profile_name)
    entries = [entry async for entry in scan]
    ...
    assert len(entries) <= 5, f"Expected up to 5 records, got {len(entries)}"
    conn_ids = [json.loads(entry.value)["connection_id"] for entry in entries]
    assert conn_ids == sorted(conn_ids, reverse=True), "Entries not sorted by connection_id descending"
...
async def test_scan_keyset_invalid_order_by(store: DBStore):
    ...
    LOGGER.debug("[test_scan_keyset_invalid_order_by] Starting keyset scan with invalid order_by")
    try:
        scan = store.scan_keyset(category="pres_ex_v20", last_id=1, limit=5, order_by="invalid_column", profile=profile_name)
        async for _ in scan:
            pass
        assert False, "Should raise DatabaseError for invalid order_by"
    except Exception as e:
...
async def test_scan_expired_records(store: DBStore):
    ...
    for entry in entries:
        assert "expiry" not in json.loads(entry.value), "Expired records should not be returned"
...
async def main():
    ...
    try:
        ...
        LOGGER.error(f"Error in test execution: %s", str(e))
