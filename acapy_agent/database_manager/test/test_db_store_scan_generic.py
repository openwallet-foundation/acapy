#poetry run python acapy_agent/database_manager/test/test_db_store_scan_generic.py
...
async def setup_data(store: DBStore, num_records: int = 50):
    ...
    LOGGER.debug(f"[setup_data] Starting insertion of {num_records} credential_record entries")
    inserted_names = []
...
async def debug_print_expiry(store: DBStore):
    """Debug: Print expiry values for credential_record entries."""
    print("Debugging: Printing expiry values for credential_record entries...")
    LOGGER.debug("[debug_print_expiry] Fetching expiry values for credential_record entries")
    async with store.session() as session:
...
async def test_scan_with_filter(store: DBStore):
    ...
    scan = store.scan(category="credential_record", tag_filter=tag_filter, profile=profile_name)
    entries = [entry async for entry in scan]
    ...
    for entry in entries:
        assert json.loads(entry.value)["status"] == "active", f"Entry {entry.name} should have status=active"
...
async def test_scan_keyset_basic(store: DBStore):
    ...
    async with store.session() as session:
        ...
        first_id = (await session.count(category="credential_record")) - len(entries) + 1  # Should be 45
    scan = store.scan_keyset(category="credential_record", last_id=first_id, limit=10, profile=profile_name)
    entries = [entry async for entry in scan]
    ...
    assert len(entries) == 4, f"Expected 4 records (cred_045 to cred_048), got {len(entries)}"  # Non-expired records after ID 45
    expected_names = [f"cred_{i:03d}" for i in range(45, 49)]
    found_names = [entry.name for entry in entries]
    assert found_names == expected_names, f"Expected names {expected_names}, got {found_names}"
    for i, entry in enumerate(entries[1:], 1):
        assert json.loads(entry.value)["credential_id"] > json.loads(entries[i-1].value)["credential_id"], "Entries not in order"
...
async def test_scan_keyset_with_filter(store: DBStore):
    ...
    tag_filter = json.dumps({"status": "active"})
    async with store.session() as session:
        entries = await session.fetch_all(category="credential_record", tag_filter=tag_filter, limit=1)
        assert len(entries) == 1, "Expected 1 active entry to get last_id"
        first_id = (await session.count(category="credential_record", tag_filter=tag_filter)) - len(entries) + 1  # Should be 15
    scan = store.scan_keyset(category="credential_record", tag_filter=tag_filter, last_id=first_id, limit=5, profile=profile_name)
    entries = [entry async for entry in scan]
    expected_count = 5  # Up to 5 active records after ID 15
    print(f"Found {len(entries)} active records with scan_keyset")
    LOGGER.debug(f"[test_scan_keyset_with_filter] Found {len(entries)} records: {[entry.name for entry in entries]}")
...
async def main():
    ...
    store = await DBStore.provision(
        uri=uri,
        pass_key="test_key",  # encrypt mode
