...
conn_str = "postgres://myuser:mypass@192.168.2.172:5432/test_generic"
...
# Sample connection JSON data (same as test_db_store_generic_normalized.py)
CONNECTION_JSON_1 = {
    "state": "active",
...
async def test_scan(store):
    ...
    scan = store.scan(category="people", tag_filter=tag_filter, limit=10, offset=0, profile=profile_name)
    ...
    for entry in entries:
        print(f" - {entry.name}: {entry.value.decode()}")

    print("Testing scan with limit and offset (people)...")
    scan_paginated = store.scan(category="people", tag_filter=tag_filter, limit=1, offset=1, profile=profile_name)
...
async def test_replace(store):
    ...
    async with store.transaction() as session:
        ...
        assert entry.value == b"{'name': 'Alice Updated'}", "Value not updated"
        ...
        assert updated_entry.value == b"{'name': 'David Updated'}", "Value not updated"
...
async def test_complex_filter(store):
    ...
    scan = store.scan(category="people", tag_filter=complex_tag_filter, profile=profile_name)
    entries = [entry async for entry in scan]
...
async def test_count_connections(store):
    ...
    async with store.session() as session:
        all_entries = await session.fetch_all(category="connection")
        count = sum(1 for entry in all_entries if json.loads(entry.value).get("state") == "active")
...
async def test_wql_exist_connections(store):
    ...
    async with store.session() as session:
        all_entries = await session.fetch_all(category="connection")
        entries = [entry for entry in all_entries if "inbound_connection_id" in json.loads(entry.value)]
        ...
        for entry in entries:
            assert "inbound_connection_id" in json.loads(entry.value), "inbound_connection_id should exist"
...
async def test_non_encrypted():
    ...
    tag_filter = json.dumps({"attr::person.status": "active"})
    scan = non_enc_store.scan(category="people", tag_filter=tag_filter, profile=profile_name)
    entries = [entry async for entry in scan]
