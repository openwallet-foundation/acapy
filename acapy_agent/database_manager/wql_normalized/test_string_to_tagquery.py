"""Unit tests for query parsing, serialization, and optimization."""

import unittest
import random
import string
from .query import (
    AndQuery, OrQuery, NotQuery, EqQuery, NeqQuery, GtQuery, GteQuery,
    LtQuery, LteQuery, LikeQuery, InQuery, ExistQuery, query_from_str, query_to_str
)
...
# Parse tests
    ...
    def test_and_with_multiple_not_eq_parse(self):
        ...
        query = query_from_str(json_str)
        expected = AndQuery([
            NotQuery(EqQuery(name1, value1)), NotQuery(EqQuery(name2, value2)),
    ...
    def test_and_with_multiple_mixed_parse(self):
        ...
        query = query_from_str(json_str)
        expected = AndQuery([
            EqQuery(name1, value1), NeqQuery(name2, value2), GtQuery(name3, value3),
            GteQuery(name4, value4), LtQuery(name5, value5), LteQuery(name6, value6),
            LikeQuery(name7, value7), InQuery(name8, [value8a, value8b]),
    ...
    def test_and_or_not_complex_case_parse(self):
        ...
        query = query_from_str(json_str)
        expected = NotQuery(AndQuery([
            EqQuery(name1, value1),
            OrQuery([
                GtQuery(name2, value2),
                NotQuery(LteQuery(name3, value3)),
                AndQuery([LtQuery(name4, value4), NotQuery(GteQuery(name5, value5))])
            ]),
