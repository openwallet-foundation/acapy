...
class TestSqliteTagEncoderNormalized(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.encoder = encoder_factory.get_encoder('sqlite', self.enc_name, self.enc_value, normalized=True)
    ...
    def run_query_and_verify(self, sql_query, params, expected_ids, test_name):
        ...
        try:
            ...
            self.assertEqual(actual_ids, expected_ids, f"{test_name} failed: Expected IDs {expected_ids}, got {actual_ids}")
        except Exception as e:
    ...
    def test_gt_positive(self):
        ...
        expected_params = ["100"]
        self.assertEqual(sql_query, expected_query, "Positive greater-than query mismatch")
        ...
        print("INSERT INTO items (id, price) VALUES "
              "(1, '090'), "
              "(2, '100'), "
              "(3, '150'), "
              "(4, '200');")
    ...
    def test_gte_positive(self):
        ...
        expected_query = "t.price >= ?"
        expected_params = ["100"]
        self.assertEqual(sql_query, expected_query, "Positive greater-than-or-equal query mismatch")
        self.assertEqual(params, expected_params, "Positive greater-than-or-equal params mismatch")
        self.verify_round_trip(query, sql_query, params)
        self.cursor.executemany(
            "INSERT INTO items (id, price) VALUES (?, ?)",
            [(1, "090"), (2, "100"), (3, "150"), (4, "200")]
        ...
        print("\n-- Cleanup\nDELETE FROM items;")
        self.run_query_and_verify(sql_query, params, [2, 3, 4], "Positive greater-than-or-equal")
    ...
    def test_gte_negated(self):
        ...
        expected_query = "NOT (t.price >= ?)"
        expected_params = ["100"]
        self.assertEqual(sql_query, expected_query, "Negated greater-than-or-equal query mismatch")
        self.assertEqual(params, expected_params, "Negated greater-than-or-equal params mismatch")
        self.verify_round_trip(query, sql_query, params)
        self.cursor.executemany(
            "INSERT INTO items (id, price) VALUES (?, ?)",
            [(1, "090"), (2, "100"), (3, "150"), (4, "200")]
    ...
    def test_lte_positive(self):
        ...
        expected_query = "t.price <= ?"
        expected_params = ["100"]
        self.assertEqual(sql_query, expected_query, "Positive less-than-or-equal query mismatch")
        self.assertEqual(params, expected_params, "Positive less-than-or-equal params mismatch")
        self.verify_round_trip(query, sql_query, params)
        self.cursor.executemany(
            "INSERT INTO items (id, price) VALUES (?, ?)",
            [(1, "090"), (2, "100"), (3, "150"), (4, "200")]
        ...
        print("\n-- Expected result: Items 1, 2")
        print("\n-- Cleanup\nDELETE FROM items;")
        self.run_query_and_verify(sql_query, params, [1, 2], "Positive less-than-or-equal")
    ...
    def test_lte_negated(self):
        ...
        expected_query = "NOT (t.price <= ?)"
        expected_params = ["100"]
        self.assertEqual(sql_query, expected_query, "Negated less-than-or-equal query mismatch")
        self.assertEqual(params, expected_params, "Negated less-than-or-equal params mismatch")
        self.verify_round_trip(query, sql_query, params)
        self.cursor.executemany(
            "INSERT INTO items (id, price) VALUES (?, ?)",
            [(1, "090"), (2, "100"), (3, "150"), (4, "200")]
    ...
    def test_like_positive(self):
        query = TagQuery.Like(TagName("field"), "%pat%")  # Use %pat% for substring matching
        wql = query.to_wql_str()
    ...
    def test_like_negated(self):
        query = TagQuery.Not(TagQuery.Like(TagName("field"), "%pat%"))  # Use %pat% for substring matching
        wql = query.to_wql_str()
    ...
    def test_and_multiple(self):
        ...
        print("INSERT INTO items (id, f1, f2) VALUES "
              "(1, 'v1', '15'), "
              "(2, 'v1', '05'), "
              "(3, 'v2', '15'), "
              "(4, 'v1', '20');")
    ...
    def test_nested_and_or(self):
        ...
        self.cursor.executemany(
            ...
            [(1, "v1", "15", "3"), (2, "v1", "05", "4"), (3, "v2", "15", "3"), (4, "v1", "05", "6")]
        )
        self.conn.commit()
        print("\n### Complete SQL Statements for Testing")
        print("CREATE TABLE items (id INTEGER PRIMARY KEY, f1 TEXT, f2 TEXT, f3 TEXT);")
        print("INSERT INTO items (id, f1, f2, f3) VALUES "
              "(1, 'v1', '15', '3'), "
              "(2, 'v1', '05', '4'), "
              "(3, 'v2', '15', '3'), "
              "(4, 'v1', '05', '6');")
    ...
    def test_comparison_conjunction(self):
        ...
        self.assertEqual(sql_query, expected_query, "Comparison conjunction query mismatch")
        self.assertEqual(params, expected_params, "Comparison conjunction params mismatch")
        self.verify_round_trip(query, sql_query, params)
        self.cursor.executemany(
            "INSERT INTO items (id, category, price) VALUES (?, ?, ?)",
            [(1, "electronics", "150"), (2, "electronics", "090"), (3, "books", "120"), (4, "electronics", "200")]
        )
        self.conn.commit()
        print("\n### Complete SQL Statements for Testing")
        print("CREATE TABLE items (id INTEGER PRIMARY KEY, category TEXT, price TEXT);")
        print("INSERT INTO items (id, category, price) VALUES "
              "(1, 'electronics', '150'), "
              "(2, 'electronics', '090'), "
              "(3, 'books', '120'), "
              "(4, 'electronics', '200');")
    ...
    def test_deeply_nested_not(self):
        ...
        self.cursor.executemany(
            ...
            [(1, "electronics", None, "in"), (2, "electronics", None, "out"), (3, None, "yes", "in"), (4, None, "yes", None)]
        )
        self.conn.commit()
        print("\n### Complete SQL Statements for Testing")
        print("CREATE TABLE items (id INTEGER PRIMARY KEY, category TEXT, sale TEXT, stock TEXT);")
        print("INSERT INTO items (id, category, sale, stock) VALUES "
    ...
    def test_and_or_not_complex_case(self):
            ...
            TagQuery.And([
                ...
                TagQuery.Or([
                    ...
                    TagQuery.And([
                        ...
                        TagQuery.Not(TagQuery.Gte(TagName("timestamp"), "2021-01-01T00:00:00"))
                    ])
                ]),
        ...
        expected_params = ["alice", "30", "180", "100", "2021-01-01T00:00:00", "abc123", "developer", "active"]
        ...
        self.conn.commit()
        expected_ids = [1, 3, 4, 5, 6, 8, 10, 11]  # bob, charlie, dave, eve, frank, hank, jack, kara
        print("\n### Complete SQL Statements for Testing")
        print("CREATE TABLE items (id INTEGER PRIMARY KEY, username TEXT, age TEXT, height TEXT, score TEXT, timestamp TEXT, secret_code TEXT, occupation TEXT, status TEXT);")
        print("INSERT INTO items (id, username, age, height, score, timestamp, secret_code, occupation, status) VALUES "
              "(1, 'bob', '25', '170', '150', '2021-02-01T00:00:00', 'xyz789', 'engineer', 'inactive'), "
              "(2, 'alice', '35', '190', '90', '2020-12-01T00:00:00', 'def456', 'developer', 'active'), "
              "(3, 'charlie', '28', '175', '120', '2021-03-01T00:00:00', 'ghi789', 'manager', 'active'), "
              "(4, 'alice', '32', '185', '95', '2020-11-01T00:00:00', 'abc123', 'developer', 'inactive'), "
        ...
        print("\n-- Cleanup\nDELETE FROM items;")
        self.run_query_and_verify(sql_query, params, expected_ids, "Complex AND/OR/NOT query")
    ...
    def test_special_characters(self):
        ...
        print("INSERT INTO items (id, f1) VALUES "
              "(1, 'val$ue'), "
              "(2, 'other'), "
              "(3, 'val$ue');")
        select_query = f"SELECT id FROM items AS t WHERE {sql_query}"
