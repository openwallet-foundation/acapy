...
class TestPostgresTagEncoderOrConj(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.normalized_encoder = encoder_factory.get_encoder('postgresql', self.enc_name, self.enc_value, normalized=True)
            self.non_normalized_encoder = encoder_factory.get_encoder('postgresql', self.enc_name, self.enc_value, normalized=False)
    ...
    def run_query_and_verify(self, sql_query, params, expected_ids, test_name, table_alias="t"):
    ...
    def test_or_conjunction_normalized(self):
        ...
        print(f"Test: OR conjunction (normalized)\nencoded query_str is: {query_str}, params: {params}")

        expected_query = "(t.category = %s OR t.price > %s)"
        expected_args = ["electronics", "100"]

        self.assertEqual(query_str, expected_query, "OR conjunction normalized query mismatch")
        self.assertEqual(params, expected_args, "OR conjunction normalized params mismatch")

        # Insert test data
        self.cursor.executemany(
            "INSERT INTO items (id, category, price) VALUES (%s, %s, %s) RETURNING id",
            [(1, "electronics", "150"), (2, "electronics", "090"), (3, "books", "120"), (4, "clothing", "200")]
        )
        self.conn.commit()

        # Run query and verify
        self.run_query_and_verify(query_str, params, [1, 2, 3, 4], "OR conjunction normalized")
    ...
    def test_or_conjunction_non_normalized(self):
        ...
        print(f"Test: OR conjunction (non-normalized)\nencoded query_str is: {query_str}, params: {params}")
        ...
        self.assertEqual(query_str, expected_query, "OR conjunction non-normalized query mismatch")
        self.assertEqual(params, expected_args, "OR conjunction non-normalized params mismatch")
        ...
        self.cursor.executemany(
            ...
            [
                ...
                (4, "category", "clothing"), (4, "price", "200")
            ]
        )
        self.conn.commit()

        # Run query and verify
        self.run_query_and_verify(query_str, params, [1, 2, 3, 4], "OR conjunction non-normalized", table_alias="i")
