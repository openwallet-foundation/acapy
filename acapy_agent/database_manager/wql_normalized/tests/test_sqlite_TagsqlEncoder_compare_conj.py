#pytest --maxfail=1 --disable-warnings --no-cov -s -vv acapy_agent/database_manager/wql_normalized/tests/test_sqlite_TagsqlEncoder_compare_conj.py
#python -m unittest acapy_agent/database_manager/wql_normalized/tests/test_sqlite_TagsqlEncoder_compare_conj.py
...
class TestTagSqlEncoder(unittest.TestCase):
    ...
    def test_comparison_conjunction(self):
        ...
        encoder = encoder_factory.get_encoder('sqlite', self.enc_name, self.enc_value, normalized=False)
        query_str = encoder.encode_query(query)
        print(f"encoded query_str is :  {query_str}")

        # note that this is a non normalize result.
        expected_query = (
