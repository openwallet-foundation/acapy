...
class TestPostgresTagEncoderNegateConj(unittest.TestCase):
    ...
    def test_negate_conj(self):
        ...
        expected_args = [
            "category", "electronics",  # From condition_1: category = electronics
            "status", "in_stock",       # From condition_1: status = in_stock
            "category", "electronics",  # From condition_2: category = electronics
            "status", "sold_out"        # From condition_2: NOT (status = sold_out)
        ]
        ...
        self.cursor.executemany(
            ...
            [
                (1, "category", "electronics"), (1, "status", "in_stock"),
                (2, "category", "electronics"), (2, "status", "sold_out"),
                (3, "category", "books"), (3, "status", "in_stock"),
                (4, "category", "clothing")
            ]
        )
        self.conn.commit()
