...
SCHEMAS = {
    "sqlite": [
        ...
        """,
        "CREATE INDEX IF NOT EXISTS idx_invitation_item_id_v0_1 ON connection_invitation_v0_1 (item_id);",
        "CREATE INDEX IF NOT EXISTS idx_invitation_connection_id_v0_1 ON connection_invitation_v0_1 (connection_id);",
        """
        CREATE TABLE IF NOT EXISTS connection_invitation_keys_v0_1 (
            ...
            FOREIGN KEY (invitation_id) REFERENCES connection_invitation_v0_1(id) ON DELETE CASCADE ON UPDATE CASCADE
        );
        """,
        "CREATE INDEX IF NOT EXISTS idx_connection_invitation_key_v0_1 ON connection_invitation_keys_v0_1 (public_key);",
        """
        CREATE TRIGGER IF NOT EXISTS trg_insert_connection_invitation_keys_v0_1
        AFTER INSERT ON connection_invitation_v0_1
        FOR EACH ROW
        BEGIN
            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT NEW.id, 'recipient', value
            FROM json_each(NEW.recipient_keys)
            WHERE NEW.recipient_keys IS NOT NULL AND NEW.recipient_keys != '' AND json_valid(NEW.recipient_keys);

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT NEW.id, 'routing', value
            FROM json_each(NEW.routing_keys)
            WHERE NEW.routing_keys IS NOT NULL AND NEW.routing_keys != '' AND json_valid(NEW.routing_keys);

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT NEW.id, 'recipient', json_extract(s.value, '$.recipientKeys[0]')
            FROM json_each(NEW.services) s
            WHERE NEW.services IS NOT NULL AND NEW.services != '' AND json_valid(NEW.services)
        ...
        FOR EACH ROW
        WHEN NEW.services IS NOT NULL AND NEW.services != '' AND json_valid(NEW.services)
             AND (NEW.recipient_keys IS NULL OR NEW.recipient_keys = '' OR NEW.endpoint IS NULL OR NEW.endpoint = '')
        BEGIN
            ...
            SELECT
                ...
                NEW.label,
                NEW.did,
                COALESCE(NEW.recipient_keys, json_extract(s.value, '$.recipientKeys'), '[]'),
                COALESCE(NEW.endpoint, json_extract(s.value, '$.serviceEndpoint')),
    ...
    "postgresql": [
        ...
        CREATE TABLE IF NOT EXISTS connection_invitation_keys_v0_1 (
            ...
            key_type TEXT NOT NULL CHECK(key_type IN ('recipient', 'routing')),
            public_key TEXT NOT NULL,
            CONSTRAINT fk_invitation_id FOREIGN KEY (invitation_id) REFERENCES connection_invitation_v0_1(id) ON DELETE CASCADE ON UPDATE CASCADE
        );
        """,
        "CREATE INDEX IF NOT EXISTS idx_connection_invitation_key_v0_1 ON connection_invitation_keys_v0_1 (public_key);",
        """
        ...
        BEGIN
            ...
            WHERE NEW.recipient_keys IS NOT NULL AND NEW.recipient_keys != '' AND NEW.recipient_keys::jsonb IS NOT NULL;

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT NEW.id, 'routing', value
            FROM jsonb_array_elements_text(NEW.routing_keys::jsonb)
            WHERE NEW.routing_keys IS NOT NULL AND NEW.routing_keys != '' AND NEW.routing_keys::jsonb IS NOT NULL;

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT NEW.id, 'recipient', jsonb_extract_path_text(s, 'recipientKeys', '0')
            FROM jsonb_array_elements(NEW.services::jsonb) s
            WHERE NEW.services IS NOT NULL AND NEW.services != '' AND NEW.services::jsonb IS NOT NULL
              AND jsonb_extract_path_text(s, 'recipientKeys', '0') IS NOT NULL;

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT NEW.id, 'routing', jsonb_extract_path_text(s, 'routingKeys', '0')
            FROM jsonb_array_elements(NEW.services::jsonb) s
            WHERE NEW.services IS NOT NULL AND NEW.services != '' AND NEW.services::jsonb IS NOT NULL
        ...
        RETURNS TRIGGER AS $$
        BEGIN
            IF NEW.services IS NOT NULL AND NEW.services != '' AND NEW.services::jsonb IS NOT NULL
               AND (NEW.recipient_keys IS NULL OR NEW.recipient_keys = '' OR NEW.endpoint IS NULL OR NEW.endpoint = '') THEN
                SELECT
                    COALESCE(NEW.recipient_keys, jsonb_extract_path_text(s, 'recipientKeys')::text, '[]') AS recipient_keys,
                    COALESCE(NEW.endpoint, jsonb_extract_path_text(s, 'serviceEndpoint')) AS endpoint
    ...
    "mssql": [
        ...
        CREATE TABLE connection_invitation_v0_1 (
            ...
            label NVARCHAR(MAX),
            did NVARCHAR(255),
            recipient_keys NVARCHAR(MAX) DEFAULT '[]',  -- JSON array of recipient public keys
        ...
        """,
        "CREATE NONCLUSTERED INDEX idx_invitation_item_id_v0_1 ON connection_invitation_v0_1 (item_id);",
        "CREATE NONCLUSTERED INDEX idx_invitation_connection_id_v0_1 ON connection_invitation_v0_1 (connection_id);",
        """
        ...
        """,
        "CREATE NONCLUSTERED INDEX idx_connection_invitation_key_v0_1 ON connection_invitation_keys_v0_1 (public_key);",
        """
        CREATE TRIGGER trg_insert_connection_invitation_keys_v0_1
        ...
        BEGIN
            ...
            WHERE i.recipient_keys IS NOT NULL AND i.recipient_keys != '' AND ISJSON(i.recipient_keys) = 1;

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT i.id, 'routing', j.value
            FROM inserted i
            CROSS APPLY OPENJSON(i.routing_keys) j
            WHERE i.routing_keys IS NOT NULL AND i.routing_keys != '' AND ISJSON(i.routing_keys) = 1;

            INSERT INTO connection_invitation_keys_v0_1 (invitation_id, key_type, public_key)
            SELECT i.id, 'recipient', JSON_VALUE(s.value, '$.recipientKeys[0]')
            FROM inserted i
            CROSS APPLY OPENJSON(i.services) s
            WHERE i.services IS NOT NULL AND i.services != '' AND ISJSON(i.services) = 1
        ...
        BEGIN
            ...
            SELECT
                ...
                i.label,
                i.did,
                COALESCE(i.recipient_keys, JSON_VALUE(s.value, '$.recipientKeys'), '[]'),
                COALESCE(i.endpoint, JSON_VALUE(s.value, '$.serviceEndpoint')),
            ...
            WHERE i.services IS NOT NULL AND i.services != '' AND ISJSON(i.services) = 1
              AND (i.recipient_keys IS NULL OR i.recipient_keys = '' OR i.endpoint IS NULL OR i.endpoint = '')
            ...
            WHERE services IS NULL OR services = '' OR ISJSON(services) = 0
               OR (recipient_keys IS NOT NULL AND recipient_keys != '' AND endpoint IS NOT NULL AND endpoint != '');
        END;
        """,
        """
        CREATE TRIGGER trg_update_connection_invitation_timestamp_v0_1
...
DROP_SCHEMAS = {
    ...
    "postgresql": [
        "DROP TRIGGER IF EXISTS trg_update_connection_invitation_timestamp_v0_1 ON connection_invitation_v0_1;",
        "DROP FUNCTION IF EXISTS update_connection_invitation_timestamp_v0_1 CASCADE;",
        "DROP TRIGGER IF EXISTS trg_populate_from_services_v0_1 ON connection_invitation_v0_1;",
        "DROP FUNCTION IF EXISTS populate_from_services_v0_1 CASCADE;",
        "DROP TRIGGER IF EXISTS trg_insert_connection_invitation_keys_v0_1 ON connection_invitation_v0_1;",
    ...
    "mssql": [
        ...
        "DROP TRIGGER IF EXISTS trg_populate_from_services_v0_1;",
        "DROP TRIGGER IF EXISTS trg_insert_connection_invitation_keys_v0_1;",
        "DROP INDEX IF EXISTS idx_connection_invitation_key_v0_1 ON connection_invitation_keys_v0_1;",
        "DROP TABLE IF EXISTS connection_invitation_keys_v0_1;",
        "DROP INDEX IF EXISTS idx_invitation_connection_id_v0_1 ON connection_invitation_v0_1;",
