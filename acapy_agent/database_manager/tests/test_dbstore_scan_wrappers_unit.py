...
class _FakeDB(AbstractDatabaseStore):
    ...
    async def rekey(self, key_method: str = None, pass_key: str = None):
        return None
    def scan(self, profile, category, tag_filter=None, offset=None, limit=None, order_by=None, descending=False):
        def gen():
            for i in range((offset or 0), (offset or 0) + (limit or 3)):
                yield types.SimpleNamespace(category=category, name=f"n{i}", value="{}", tags={})
        return gen()
    def scan_keyset(self, profile, category, tag_filter=None, last_id=None, limit=None, order_by=None, descending=False):
        def gen():
            start = (last_id or 0) + 1
            for i in range(start, start + (limit or 3)):
                yield types.SimpleNamespace(category=category, name=f"k{i}", value="{}", tags={})
        return gen()
...
class _FakeSession(AbstractDatabaseSession):
    ...
    async def fetch_all(self, category: str, tag_filter: str | dict = None, limit: int = None, for_update: bool = False, order_by: str | None = None, descending: bool = False):
        return [types.SimpleNamespace(category=category, name=f"n{i}", value="{}", tags={}) for i in range(3)]
    async def insert(self, category: str, name: str, value: str | bytes = None, tags: dict = None, expiry_ms: int = None, value_json=None):
        return None
    async def replace(self, category: str, name: str, value: str | bytes = None, tags: dict = None, expiry_ms: int = None, value_json=None):
        return None
