...
async def test_sqlite_open_missing_default_profile(tmp_path):
    ...
    cur.execute("INSERT INTO config(name,value) VALUES('schema_release_number','release_0')")
    conn.commit()
    conn.close()
    ...
    with pytest.raises(DatabaseError) as e:
        cfg.open(profile="p")
    assert e.value.code in {DatabaseErrorCode.DEFAULT_PROFILE_NOT_FOUND, DatabaseErrorCode.QUERY_ERROR}
