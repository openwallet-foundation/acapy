...
async def test_sqlite_config_open_profile_missing(tmp_path):
    ...
    pool, profile, path, rel = cfg.provision(profile="p", recreate=True, release_number="release_0")
    pool.close()
    with pytest.raises(DatabaseError) as e:
        cfg.open(profile="other")
    assert e.value.code in {DatabaseErrorCode.PROFILE_NOT_FOUND, DatabaseErrorCode.QUERY_ERROR}
