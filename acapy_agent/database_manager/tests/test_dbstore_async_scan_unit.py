...
class _AsyncDB(AbstractDatabaseStore):
    ...
    async def rekey(self, key_method: str = None, pass_key: str = None):
        return None
    async def scan(self, profile, category, tag_filter=None, offset=None, limit=None, order_by=None, descending=False):
        for i in range((offset or 0), (offset or 0) + (limit or 2)):
            yield types.SimpleNamespace(category=category, name=f"an{i}", value="{}", tags={})
    async def scan_keyset(self, profile, category, tag_filter=None, last_id=None, limit=None, order_by=None, descending=False):
        start = (last_id or 0) + 1
        for i in range(start, start + (limit or 2)):
            yield types.SimpleNamespace(category=category, name=f"ak{i}", value="{}", tags={})
...
class _AsyncSession(AbstractDatabaseSession):
    ...
    async def fetch_all(self, category: str, tag_filter: str | dict = None, limit: int = None, for_update: bool = False, order_by: str | None = None, descending: bool = False):
        return [types.SimpleNamespace(category=category, name=f"an{i}", value="{}", tags={}) for i in range(2)]
