...
async def test_sqlite_open_target_release_mismatch(tmp_path):
    ...
    pool, profile, path, rel = cfg.provision(profile="p", recreate=True, release_number="release_0_1")
    pool.close()

    cfg2 = SqliteConfig(uri=f"sqlite://{db_path}")
    with pytest.raises(DatabaseError) as e:
        cfg2.open(profile="p", schema_migration=None, target_schema_release_number="release_0_2")
    assert e.value.code in {DatabaseErrorCode.UNSUPPORTED_VERSION, DatabaseErrorCode.QUERY_ERROR}
