...
class _OkBackend(DatabaseBackend):
    def __init__(self):
        self._db = _OkDB()
    def provision(self, uri, key_method, pass_key, profile, recreate, release_number: str = "release_0", schema_config: str = None, config: dict | None = None):
        return self._db
    def open(self, uri, key_method, pass_key, profile, schema_migration: bool | None = None, target_schema_release_number: str | None = None, config: dict | None = None):
        return self._db
