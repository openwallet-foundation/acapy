...
class Scan(AsyncIterator):
    def __init__(self, store, profile, category, tag_filter, offset, limit, order_by, descending):
        self._store = store
        ...
        self._is_async = (inspect.iscoroutinefunction(self._store._db.scan) or 
                          inspect.isasyncgenfunction(self._store._db.scan))
    ...
    async def __anext__(self):
        if self._generator is None:
            if self._is_async:
                ...
                self._generator = await loop.run_in_executor(self._executor, create_generator)
...
class ScanKeyset(AsyncIterator):
    def __init__(
        ...
        self._is_async = (inspect.iscoroutinefunction(self._store._db.scan_keyset) or 
                          inspect.isasyncgenfunction(self._store._db.scan_keyset))
...
class DBStore:
    def __init__(self, db: AbstractDatabaseStore, uri: str, release_number: str = "release_0"):
        LOGGER.debug(f"Store initialized with db={db}, uri={uri}, release_number={release_number}")
        self._db = db
        self._uri = uri
    ...
    async def provision(
        ...
        *,
        ...
        if not _backend_registry:  # Register backends if not already done
            backend_registration = importlib.import_module(".databases.backends.backend_registration", package=__package__)
            backend_registration.register_backends()
        db_type = uri.split(":")[0]
        backend = _backend_registry.get(db_type)
        if not backend:
            raise DBStoreError(DBStoreErrorCode.BACKEND, f"Unsupported database type: {db_type}")
        try:
            if inspect.iscoroutinefunction(backend.provision):
                db = await backend.provision(
                    uri, key_method, pass_key, profile, recreate, release_number, schema_config, config=config
                )
            else:
                db = await asyncio.to_thread(
                    backend.provision, uri, key_method, pass_key, profile, recreate, release_number, schema_config, config=config
                )
        ...
        try:
            if inspect.iscoroutinefunction(backend.open):
                db = await backend.open(
                    uri, key_method, pass_key, profile, schema_migration, target_schema_release_number, config=config
                )
            else:
                db = await asyncio.to_thread(
                    backend.open, uri, key_method, pass_key, profile, schema_migration, target_schema_release_number, config=config
                )
        except Exception as e:
    ...
    @classmethod
    async def remove(cls, uri: str, release_number: str = "release_0", config: Optional[dict] = None) -> bool:
        """Remove the database store."""
        LOGGER.debug(f"remove called with uri={uri}, release_number={release_number}, config={config}")
        if not _backend_registry:  # Register backends if not already done
            backend_registration = importlib.import_module(".databases.backends.backend_registration", package=__package__)
            backend_registration.register_backends()
        db_type = uri.split(":")[0]
    ...
    def scan(
        ...
        return Scan(self, profile, category, tag_filter, offset, limit, order_by, descending)
    ...
    def scan_keyset(
        ...
        return ScanKeyset(self, profile, category, tag_filter, last_id, limit, order_by, descending)
...
class DBStoreSession:
    ...
    async def fetch(self, category: str, name: str, *, for_update: bool = False) -> Optional[Entry]:
        LOGGER.debug(f"fetch called with category={category}, name={name}, for_update={for_update}")
        try:
            return await self._db_session.fetch(category, name, tag_filter=None, for_update=for_update)
        except Exception as e:
    ...
    async def insert(
        self, category: str, name: str, value: Union[str, bytes] = None,
    ...
    async def remove_all(self, category: str, tag_filter: Union[str, dict] = None) -> int:
        LOGGER.debug(f"remove_all called with category={category}, tag_filter={tag_filter}")
        try:
...
class DBOpenSession:
    def __init__(self, db: AbstractDatabaseStore, profile: Optional[str], is_txn: bool, release_number: str):
        LOGGER.debug(f"OpenSession initialized with db={db}, profile={profile}, is_txn={is_txn}, release_number={release_number}")
        self._db = db
        self._profile = profile
        self._is_txn = is_txn
        self._release_number = release_number
    ...
    async def _open(self) -> DBStoreSession:
        ...
        self._db_session = await method(self._profile) if inspect.iscoroutinefunction(method) else method(self._profile)
