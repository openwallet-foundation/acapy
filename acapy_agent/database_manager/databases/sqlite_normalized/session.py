...
class SqliteSession(AbstractDatabaseSession):
    def __init__(self, database: SqliteDatabase, profile: str, is_txn: bool, release_number: str = "release_0_1"):
    ...
    async def __aenter__(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.conn = await asyncio.to_thread(self.pool.get_connection, timeout=60.0)
                try:
                    cursor = await asyncio.to_thread(self.conn.cursor)
                    await asyncio.to_thread(cursor.execute, "SELECT 1")
                    if not hasattr(self.pool, 'encryption_key') or not self.pool.encryption_key:
                ...
                if self.profile_id is None:
                    self.profile_id = await asyncio.to_thread(self._get_profile_id, self.profile)
                if self.is_txn:
    ...
    async def insert(self, category: str, name: str, value: Union[str, bytes] = None, 
        ...
        def _insert():
            with self.lock:
                try:
                    cursor = self.conn.cursor()
                    handler.insert(cursor, self.profile_id, category, name, value, tags or {}, expiry_ms)
                    if not self.is_txn:
                        self.conn.commit()
                except asyncio.exceptions.CancelledError:
                    if not self.is_txn:
    ...
    async def fetch(self, category: str, name: str, tag_filter: Union[str, dict] = None, 
        ...
        def _fetch():
            with self.lock:
                try:
                    cursor = self.conn.cursor()
                    return handler.fetch(cursor, self.profile_id, category, name, tag_filter, for_update)
                except asyncio.exceptions.CancelledError:
                    raise
                except Exception as e:
    ...
    async def fetch_all(self, category: str, tag_filter: Union[str, dict] = None, 
        ...
        def _fetch_all():
            with self.lock:
                try:
                    cursor = self.conn.cursor()
                    return handler.fetch_all(cursor, self.profile_id, category, tag_filter, limit, for_update, order_by, descending)
                except asyncio.exceptions.CancelledError:
                    raise
                except Exception as e:
    ...
    async def replace(self, category: str, name: str, value: Union[str, bytes] = None, 
        ...
        def _replace():
            with self.lock:
                try:
                    cursor = self.conn.cursor()
                    handler.replace(cursor, self.profile_id, category, name, value, tags or {}, expiry_ms)
                    if not self.is_txn:
                        self.conn.commit()
                except asyncio.exceptions.CancelledError:
                    if not self.is_txn:
    ...
    async def remove_all(self, category: str, tag_filter: Union[str, dict] = None) -> int:
        ...
        def _remove_all():
            with self.lock:
                try:
                    cursor = self.conn.cursor()
                    result = handler.remove_all(cursor, self.profile_id, category, tag_filter)
                    if not self.is_txn:
                        self.conn.commit()
                    return result
                except asyncio.exceptions.CancelledError:
