...
class NormalizedHandler(BaseHandler):
    ...
    def __init__(self, category: str, columns: List[str], table_name: Optional[str] = None):
        super().__init__(category)
        self.table = table_name or category
        self.columns = columns
        self.ALLOWED_ORDER_BY_COLUMNS = set(columns) | {"id", "name", "value"}
        self.encoder = encoder_factory.get_encoder("sqlite", lambda x: x, lambda x: x, normalized=True)
        LOGGER.debug(f"[init] Initialized NormalizedHandler for category={category}, table={self.table}, columns={columns}")

    def insert(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
               value: Union[str, bytes], tags: dict, expiry_ms: int) -> None:
        LOGGER.debug(f"[insert] Starting with category={category}, name={name}, value={value}, tags={tags}, expiry_ms={expiry_ms}")
        ...
        LOGGER.debug(f"[insert] Inserting into items table with profile_id={profile_id}, category={category}, name={name}, value={value}, expiry={expiry}")
        cursor.execute("""
        ...
        for col in self.columns:
            if col in json_data:
                val = json_data[col]
                LOGGER.debug(f"[insert] Column {col} found in json_data with value {val} (type: {type(val)})")
                if col == 'pres_request':
                    ...
                    if isinstance(val, str) and is_valid_json(val):
                        try:
                            ...
                            LOGGER.debug(f"[insert] Force serialized {col} to JSON: {val}")
                        except json.JSONDecodeError as e:
                            LOGGER.error(f"[insert] Failed to re-serialize pres_request: {str(e)}, raw value: {val}")
                            raise DatabaseError(
                                code=DatabaseErrorCode.QUERY_ERROR,
                        ...
                        try:
                            ...
                            LOGGER.error(f"[insert] Serialization failed for column {col}: {str(e)}")
                            raise
                ...
                val = tags[col]
                LOGGER.debug(f"[insert] Column {col} found in tags with value {val} (type: {type(val)})")
                ...
                LOGGER.warning(f"[insert] Column {col} not found in json_data or tags, setting to NULL")
                data[col] = None
        ...
        try:
            ...
            LOGGER.debug(f"[insert] Successfully inserted into {self.table} for item_id={item_id}")
        except sqlite3.OperationalError as e:
            LOGGER.error(f"[insert] SQLite error during insert into {self.table}: {str(e)}")
    ...
    def replace(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
                value: Union[str, bytes], tags: dict, expiry_ms: int) -> None:
        """Replace an existing entry in the database."""
        LOGGER.debug(f"[replace] Replacing record with category={category}, name={name}, value={value}, tags={tags}")
        ...
        row = cursor.fetchone()
        if not row:
            LOGGER.error(f"[replace] Record not found for category={category}, name={name}")
            raise DatabaseError(
        ...
        LOGGER.debug(f"[replace] Updating items table with value={value}, expiry={expiry}, item_id={item_id}")
        cursor.execute("""
        ...
        LOGGER.debug(f"[replace] Deleting existing entry from {self.table} for item_id={item_id}")
        ...
        for col in self.columns:
            if col in json_data:
                val = json_data[col]
                LOGGER.debug(f"[replace] Column {col} found in json_data with value {val} (type: {type(val)})")
                if col == 'pres_request':
                    ...
                    if isinstance(val, str) and is_valid_json(val):
                        try:
                            val = json.loads(val)
                            val = serialize_json_with_bool_strings(val)
                            LOGGER.debug(f"[replace] Force serialized {col} to JSON: {val}")
                        except json.JSONDecodeError as e:
                            LOGGER.error(f"[replace] Failed to re-serialize pres_request: {str(e)}, raw value: {val}")
                            raise DatabaseError(
                                code=DatabaseErrorCode.QUERY_ERROR,
                                message=f"Failed to re-serialize pres_request: {str(e)}"
                            )
                        ...
                        try:
                            ...
                            LOGGER.error(f"[replace] Serialization failed for column {col}: {str(e)}")
                            raise
                elif isinstance(val, (dict, list)):
                    try:
                        val = serialize_json_with_bool_strings(val)
                ...
                val = tags[col]
                LOGGER.debug(f"[replace] Column {col} found in tags with value {val} (type: {type(val)})")
                if isinstance(val, (dict, list)):
                    try:
                        val = serialize_json_with_bool_strings(val)
                        LOGGER.debug(f"[replace] Serialized {col} to JSON: {val}")
                ...
                LOGGER.warning(f"[replace] Column {col} not found in json_data or tags, setting to NULL")
                data[col] = None
        ...
        try:
            cursor.execute(sql, list(data.values()))
            LOGGER.debug(f"[replace] Successfully inserted into {self.table} for item_id={item_id}")
        except sqlite3.OperationalError as e:
            LOGGER.error(f"[replace] SQLite error during insert into {self.table}: {str(e)}")
    ...
    def fetch(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
        ...
        LOGGER.debug(f"[fetch] Executing SQL: {base_query.strip()} | Params: {base_params}")
    ...
    def fetch_all(self, cursor: sqlite3.Cursor, profile_id: int, category: str, 
        ...
        LOGGER.debug("[%s] Starting with profile_id=%d, category=%s, tag_filter=%s, limit=%s, for_update=%s, order_by=%s, descending=%s",
                     operation_name, profile_id, category, tag_filter, limit, for_update, order_by, descending)
        ...
        LOGGER.debug(f"[fetch_all] Executing SQL: {query.strip()} | Params: {full_params}")
        cursor.execute(query, full_params)
        columns = [desc[0] for desc in cursor.description]
        entries = []
    ...
    def count(self, cursor: sqlite3.Cursor, profile_id: int, category: str, 
        ...
        query = f"""
            ...
            AND {sql_clause}
        """
        LOGGER.debug(f"[count] Executing SQL: {query.strip()} | Params: {[profile_id, category] + params}")
        cursor.execute(query, [profile_id, category] + params)
        count = cursor.fetchone()[0]
        LOGGER.debug(f"[count] Counted {count} entries")
        return count

    def remove(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str) -> None:
        ...
        row = cursor.fetchone()
        if not row:
            LOGGER.error(f"[remove] Record not found for category={category}, name={name}")
            raise DatabaseError(
                code=DatabaseErrorCode.RECORD_NOT_FOUND,
                message=f"Record not found for category '{category}' and name '{name}'"
            )
    ...
    def remove_all(self, cursor: sqlite3.Cursor, profile_id: int, category: str, 
                   tag_filter: Union[str, dict]) -> int:
        """Remove all entries matching the specified criteria."""
        LOGGER.debug(f"[remove_all] Removing all records with category={category}, tag_filter={tag_filter}")
        sql_clause = "1=1"
        params = []
        if tag_filter:
            if isinstance(tag_filter, str):
        ...
        """
        LOGGER.debug(f"[remove_all] Executing SQL: {query.strip()} | Params: {[profile_id, category] + params}")
        cursor.execute(query, [profile_id, category] + params)
    ...
    def scan(
        ...
        operation_name = "scan"
        LOGGER.debug(f"[{operation_name}] Scanning records with category={category}, offset={offset}, limit={limit}, order_by={order_by}, descending={descending}")
        if order_by and order_by not in self.ALLOWED_ORDER_BY_COLUMNS:
        ...
        try:
            ...
            table_prefix = "t" if order_by in self.columns else "i"
            order_direction = "DESC" if descending else "ASC"
            LOGGER.debug(f"[{operation_name}] Using ORDER BY {table_prefix}.{order_column} {order_direction}")
            ...
            if limit is not None or offset is not None:
                subquery += " LIMIT ?"
                params.append(limit if limit is not None else -1)  #  just use -1 for no limit
    ...
    def scan_keyset(
        ...
        LOGGER.debug(f"[{operation_name}] Starting with profile_id={profile_id}, category={category}, tag_query={tag_query}, last_id={last_id}, limit={limit}, order_by={order_by}, descending={descending}, table={self.table}")
        ...
        try:
            ...
            table_prefix = "t" if order_by in self.columns else "i"
            order_direction = "DESC" if descending else "ASC"
            keyset_clause = f"AND {table_prefix}.{order_column} > ?" if last_id is not None else ""
    ...
    def get_sql_clause(self, tag_query: TagQuery) -> Tuple[str, List[Any]]:
        ...
        LOGGER.debug(f"[get_sql_clause] Generated SQL clause: {sql_clause} with arguments: {arguments}")
