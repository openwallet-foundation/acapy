from ..normalized_handler import NormalizedHandler, is_valid_json, serialize_json_with_bool_strings, deserialize_tags
...
class CredExV20CustomHandler(NormalizedHandler):
    ...
    def __init__(self, category: str, columns: List[str], table_name: Optional[str] = None, release_number: str = "release_1", db_type: str = "sqlite"):
        super().__init__(category, columns, table_name)
        self.version = self._get_version()
        LOGGER.debug(f"Initialized CredExV20CustomHandler for category={category}, table={self.table}, columns={columns}, release_number={release_number}, db_type={db_type}, version={self.version}")
    ...
    def _get_version(self) -> str:
        ...
        try:
            ...
            LOGGER.warning(f"Table name {self.table} does not match expected format, defaulting to version 1")
            return "1"
    ...
    def _extract_cred_def_id(self, json_data: dict) -> str:
        try:
            ...
            offers_attach = cred_offer.get('offers_attach', []) or cred_offer.get('offers~attach', [])
            if not offers_attach or not isinstance(offers_attach, list):
                return None
            for attachment in offers_attach:
                if attachment.get('@id') == 'anoncreds' and attachment.get('mime-type') == 'application/json':
    ...
    def _extract_attributes_and_formats(self, json_data: dict, cred_ex_id: int, cursor: sqlite3.Cursor):
        ...
        for field in ['cred_proposal', 'cred_offer', 'cred_issue']:
            if field in json_data and json_data[field] and not attributes:
                try:
                    ...
                    if 'credential_preview' in data and 'attributes' in data['credential_preview']:
                        attributes = data['credential_preview']['attributes']
                        LOGGER.debug(f"[extract] Extracted attributes from {field}: {attributes}")
                        break
                except json.JSONDecodeError as e:
                    LOGGER.warning(f"[extract] Invalid {field} JSON: {str(e)}")
                except Exception as e:
                    LOGGER.warning(f"[extract] Error extracting attributes from {field}: {str(e)}")

        # Insert attributes into dynamic attributes subtable
        attributes_table = f"cred_ex_v20_attributes_v{self.version}"
        try:
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?", (attributes_table,))
            ...
            for attr in attributes:
                if 'name' in attr and 'value' in attr:
                    ...
                    LOGGER.debug(f"[extract] Inserted attribute: name={attr['name']}, value={attr['value']} for cred_ex_v20_id={cred_ex_id}")
        except sqlite3.OperationalError as e:
            LOGGER.error(f"[extract] SQLite error inserting into {attributes_table}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
            ...
            if field in json_data and json_data[field]:
                try:
                    ...
                    if 'formats' in data:
                        ...
                        LOGGER.debug(f"[extract] Extracted formats from {field}: {formats}")
                except json.JSONDecodeError as e:
                    LOGGER.warning(f"[extract] Invalid {field} JSON for formats: {str(e)}")
                except Exception as e:
                    LOGGER.warning(f"[extract] Error extracting formats from {field}: {str(e)}")

        # Insert formats into dynamic formats subtable
        formats_table = f"cred_ex_v20_formats_v{self.version}"
        try:
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?", (formats_table,))
            if not cursor.fetchone():
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
            ...
            for fmt in formats:
                if 'attach_id' in fmt:
                    ...
                    LOGGER.debug(f"[extract] Inserted format: attach_id={fmt['attach_id']}, format_type={fmt.get('format')} for cred_ex_v20_id={cred_ex_id}")
        except sqlite3.OperationalError as e:
            LOGGER.error(f"[extract] SQLite error inserting into {formats_table}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
    ...
    def insert(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
               value: Union[str, bytes, dict], tags: dict, expiry_ms: int) -> None:
        import traceback
        LOGGER.setLevel(logging.DEBUG)
        LOGGER.debug(f"[insert] Starting with category={category}, name={name}, thread_id={tags.get('thread_id')}, stack={''.join(traceback.format_stack(limit=5))}")

        expiry = None
        if expiry_ms:
            expiry = (datetime.now(timezone.utc) + timedelta(milliseconds=expiry_ms)).isoformat()
        ...
        try:
            ...
            if existing_item:
                ...
                LOGGER.debug(f"[insert] Found existing item_id={item_id} for category={category}, name={name}")
                cursor.execute(f"SELECT id, thread_id FROM {self.table} WHERE item_id = ?", (item_id,))
                existing_cred = cursor.fetchone()
                if existing_cred:
                    LOGGER.error(f"[insert] Duplicate cred_ex_v20 record for item_id={item_id}, thread_id={existing_cred[1]}")
                    raise DatabaseError(
            ...
            if tags.get('thread_id'):
                cursor.execute(f"SELECT id, item_id FROM {self.table} WHERE thread_id = ?", (tags.get('thread_id'),))
                duplicates = cursor.fetchall()
                if duplicates:
                    LOGGER.error(f"[insert] Duplicate thread_id found in {self.table}: {duplicates}")
                    raise DatabaseError(
                        code=DatabaseErrorCode.DUPLICATE_ITEM_ENTRY_ERROR,
            ...
            for col in self.columns:
                if col == 'cred_def_id' and cred_def_id:
                    ...
                    LOGGER.debug(f"[insert] Added column {col} from custom extraction: {cred_def_id}")
                    ...
                    data[col] = None
                    LOGGER.debug(f"[insert] Column {col} not found in json_data or tags, setting to NULL")
            ...
            LOGGER.debug(f"[insert] Inserted cred_ex_v20 record with id={cred_ex_id}, item_id={item_id}, thread_id={tags.get('thread_id')}")
            ...
            LOGGER.error(f"[insert] SQLite error during insert for item_id={item_id}, thread_id={tags.get('thread_id')}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
                message=f"SQLite error during insert: {str(e)}"
            )
        except Exception as e:
            LOGGER.error(f"[insert] Unexpected error during insert for item_id={item_id}, thread_id={tags.get('thread_id')}: {str(e)}")
            raise
    ...
    def replace(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
        ...
        import traceback
        LOGGER.setLevel(logging.DEBUG)
        LOGGER.debug(f"[replace] Starting with category={category}, name={name}, thread_id={tags.get('thread_id')}, stack={''.join(traceback.format_stack(limit=5))}")
        ...
        try:
            ...
            if tags.get('thread_id'):
                cursor.execute(f"SELECT id, item_id FROM {self.table} WHERE thread_id = ? AND item_id != ?", 
                              (tags.get('thread_id'), item_id))
                duplicates = cursor.fetchall()
                if duplicates:
                    LOGGER.warning(f"[replace] Duplicate thread_id found in {self.table}: {duplicates}")
                    for dup_id, dup_item_id in duplicates:
                        cursor.execute(f"DELETE FROM {self.table} WHERE id = ?", (dup_id,))
                        LOGGER.debug(f"[replace] Deleted duplicate record id={dup_id}, thread_id={tags.get('thread_id')}")
            ...
            if 'cred_issue' in json_data and json_data['cred_issue']:
                ...
                if isinstance(cred_issue, str) and is_valid_json(cred_issue):
                    ...
                    LOGGER.debug("[replace] cred_issue is already a dict, no further validation needed")
                else:
                    raise DatabaseError(
                        code=DatabaseErrorCode.QUERY_ERROR,
            ...
            for col in self.columns:
                if col == 'cred_def_id' and cred_def_id:
                    data[col] = cred_def_id
                    LOGGER.debug(f"[replace] Added column {col} from custom extraction: {cred_def_id}")
                elif col in json_data:
                    val = json_data[col]
                    if isinstance(val, (dict, list)):
                        val = serialize_json_with_bool_strings(val)
                    ...
                    data[col] = None
                    LOGGER.debug(f"[replace] Column {col} not found in json_data or tags, setting to NULL")
            ...
            cursor.execute(sql, list(data.values()))
            cred_ex_id = cursor.lastrowid
            LOGGER.debug(f"[replace] Inserted cred_ex_v20 record with id={cred_ex_id}, item_id={item_id}, thread_id={tags.get('thread_id')}")

            # Extract and insert attributes and formats
            self._extract_attributes_and_formats(json_data, cred_ex_id, cursor)

        except (sqlite3.OperationalError, sqlite3.IntegrityError, sqlite3.DatabaseError) as e:
            LOGGER.error(f"[replace] SQLite error during replace for item_id={item_id}, thread_id={tags.get('thread_id')}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
                message=f"SQLite error during replace: {str(e)}"
            )
        except Exception as e:
            LOGGER.error(f"[replace] Unexpected error during replace for item_id={item_id}, thread_id={tags.get('thread_id')}: {str(e)}")
            raise