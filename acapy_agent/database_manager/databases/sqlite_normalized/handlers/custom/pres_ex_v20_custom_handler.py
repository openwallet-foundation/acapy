...
class PresExV20CustomHandler(NormalizedHandler):
    ...
    def __init__(self, category: str, columns: List[str], table_name: Optional[str] = None):
        ...
        LOGGER.debug(f"Initialized PresExV20CustomHandler for category={category}, table={table_name or category}, columns={columns}")
    ...
    def insert(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
        ...
        LOGGER.debug(f"Inserting record with category={category}, name={name}, value={value}, tags={tags}")
        ...
        LOGGER.debug(f"Added revealed_attr_groups to json_data: {json_data['revealed_attr_groups']}")

        LOGGER.debug(f"Inserting into items table with profile_id={profile_id}, category={category}, name={name}, value={value}, expiry={expiry}")
        cursor.execute("""
        ...
        for col in self.columns:
            if col in json_data:
                val = json_data[col]
                LOGGER.debug(f"Column {col} found in json_data with value {val} (type: {type(val)})")
                if col == 'pres_request':
                    if isinstance(val, str) and is_valid_json(val):
                        try:
                            ...
                            LOGGER.error(f"Failed to re-serialize pres_request: {str(e)}, raw value: {val}")
                            raise DatabaseError(
                                code=DatabaseErrorCode.QUERY_ERROR,
                ...
                val = tags[col]
                LOGGER.debug(f"Column {col} found in tags with value {val} (type: {type(val)})")
    ...
    def replace(self, cursor: sqlite3.Cursor, profile_id: int, category: str, name: str, 
        ...
        LOGGER.debug(f"Replacing record with category={category}, name={name}, value={value}, tags={tags}")
        ...
        LOGGER.debug(f"Updating items table with value={value}, expiry={expiry}, item_id={item_id}")
        cursor.execute("""
