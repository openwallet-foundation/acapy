...
async def minimal_test():
    store = None
    try:
        config = SqliteConfig(uri="sqlite://test.db", encryption_key="strong_key", pool_size=5, schema_config="generic")
        pool, profile_name, path, effective_release_number = config.provision(profile="test_profile", recreate=True, release_number="release_0")
        ...
        config_new = SqliteConfig(uri="sqlite://test.db", encryption_key="new_secure_key", pool_size=5, schema_config="generic")
        pool, profile_name, path, effective_release_number = config_new.provision(profile="test_profile", recreate=False, release_number="release_0")
        store = SqliteDatabase(pool, profile_name, path, effective_release_number)
        ...
        if store is not None:
            LOGGER.debug(f"Closing store in finally: {store}, type={type(store)}, id={id(store)}")
            try:
                ...
                LOGGER.error(f"Error closing store: {str(close_err)}, store={store}, type={type(store)}")
        else:
