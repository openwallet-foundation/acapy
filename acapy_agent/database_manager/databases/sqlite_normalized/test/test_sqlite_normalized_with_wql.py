"""
poetry run python acapy_agent/database_manager/databases/sqlite_normalized/test/test_sqlite_normalized_with_wql.py
                              

This script tests the functionality of the SQLite database for the 'connection' category with WQL queries.
...
from acapy_agent.database_manager.databases.sqlite_normalized.database import SqliteDatabase
from acapy_agent.database_manager.databases.backends.backend_registration import register_backends
import sqlite3
try:
...
async def run_tests(store, db_path, config_new, is_encrypted=True):
    try:
        ...
        async with store.session() as session:
            ...
            print(f"Connections before tests: {[f'{entry.name}: {entry.tags}, value={json.loads(entry.value)}' for entry in entries]}")
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="connection", tag_filter=wql_equality, limit=2))
        ...
        for entry in scanned_entries:
            ...
            assert json.loads(entry.value)["state"] == "active", "State should be 'active'"
        ...
        async with store.transaction() as session:
            print("Updating Connection 4..." if not is_encrypted else "Updating Connection 1...")
            ...
            updated_entry = await session.fetch(category="connection", name="conn_4" if not is_encrypted else "conn_1")
            print(f"Updated Connection {'4' if not is_encrypted else '1'}: {json.loads(updated_entry.value)}")
            assert json.loads(updated_entry.value)["state"] == "completed", "State not updated"

            print("Inserting Connection 4..." if is_encrypted else "Inserting Connection 7...")
            await session.insert(
                category="connection",
                name="conn_4" if is_encrypted else "conn_7",
                value=json.dumps(CONNECTION_JSON_1),
                tags={"state": CONNECTION_JSON_1["state"], "alias": CONNECTION_JSON_1["alias"]}
            )
            new_entry = await session.fetch(category="connection", name="conn_4" if is_encrypted else "conn_7")
            print(f"Inserted Connection {'4' if is_encrypted else '7'}: {json.loads(new_entry.value)}")
            assert new_entry is not None, "Insert failed"

            print("Updating Connection 4..." if is_encrypted else "Updating Connection 7...")
            ...
            updated_conn4 = await session.fetch(category="connection", name="conn_4" if is_encrypted else "conn_7")
            print(f"Updated Connection {'4' if not is_encrypted else '7'}: {json.loads(updated_conn4.value)}")
        ...
        async with store.session() as session:
            ...
            for entry in entries:
                if entry.name == conn_id:
                    ...
                    print(f"Found other connection {entry.name}: {json.loads(entry.value)}")

        # Step 6: Test fetch with WQL filters
        print("\n### Testing Fetch with WQL Filters ###")
        async with store.session() as session:
            print("Fetching conn_1 with state='completed'...")
            entry = await session.fetch(category="connection", name="conn_1" if is_encrypted else "conn_4", tag_filter=json.dumps({"state": "completed"}))
            ...
            entry = await session.fetch(category="connection", name="conn_1" if is_encrypted else "conn_4", tag_filter=json.dumps({"state": "active"}))
            assert entry is None, "Should not fetch conn_1 with state='active'"

            print("Fetching conn_2 with {'$exist': ['alias']}...")
            LOGGER.debug("Executing WQL query: %s for conn_2", json.dumps({"$exist": ["alias"]}))
            entry = await session.fetch(category="connection", name="conn_2" if is_encrypted else "conn_5", tag_filter=json.dumps({"$exist": ["alias"]}))
            assert entry is None, "Should not fetch conn_2 since alias is None"
            LOGGER.debug("Result for conn_2 $exist query: %s", entry)

            print(f"Fetching {'conn_3' if is_encrypted else 'conn_6'} with {json.dumps({'$exist': ['alias']})}...")
            LOGGER.debug("Executing WQL query: %s for %s", json.dumps({"$exist": ["alias"]}), "conn_3" if is_encrypted else "conn_6")
            entry = await session.fetch(category="connection", name="conn_3" if is_encrypted else "conn_6", tag_filter=json.dumps({"$exist": ["alias"]}))
            ...
            entry = await session.fetch(category="connection", name="conn_1" if is_encrypted else "conn_4", tag_filter=json.dumps({"created_at": {"$lt": "2025-05-08T00:00:00Z"}}))
            assert entry is not None, "Should fetch conn_1 with created_at < '2025-05-08T00:00:00Z'"
            print(f"Fetched: {entry.name} with created_at={json.loads(entry.value)['created_at']}")

            print(f"Fetching {'conn_3' if is_encrypted else 'conn_6'} with created_at < '2025-05-08T00:00:00Z'...")
            entry = await session.fetch(category="connection", name="conn_3" if is_encrypted else "conn_6", tag_filter=json.dumps({"created_at": {"$lt": "2025-05-08T00:00:00Z"}}))
            assert entry is None, "Should not fetch conn_3/conn_6 with created_at < '2025-05-08T00:00:00Z'"
        ...
        async with store.session() as session:
            ...
            for entry in entries:
                print(f" - {entry.name}: {json.loads(entry.value)}")
                assert json.loads(entry.value)["created_at"] > "2025-05-08T00:00:00Z", "Date should be after 2025-05-08"
        ...
        async with store.transaction() as session:
            deleted_count = await session.remove_all(category="connection", tag_filter=wql_remove)
        ...
        if is_encrypted:
            print("\n### Testing the Key ###")
            print(f"Trying to access the database with {'new_secure_key' if is_encrypted else 'no key'}...")
            async with store.session() as session:
                count = await session.count(category="connection")
                print(f"Counted {count} connections with {'new key' if is_encrypted else 'no key'}")
        ...
        if is_encrypted:
            ...
            config_old = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="strong_key", pool_size=5, schema_config="normalize")
        ...
        print("\n### Working with Data ###")
        print(f"Using the database with {'new_secure_key' if is_encrypted else 'no key'}...")
        async with store.session() as session:
            entries = await session.fetch_all(category="connection")
...
async def main():
    register_backends()
    print("Starting the SQLite database test program for 'connection' category with WQL queries (Asyncio Version)...")
    store = None
    non_enc_store = None
    store_old = None
    store_with_key = None
    try:
        ...
        print("Provisioning the database at", db_path, "with encryption key 'strong_key'...")
        config = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="strong_key", pool_size=5, schema_config="normalize")
        ...
        async with store.transaction() as session:
            ...
            await session.insert(
                ...
                tags={"state": CONNECTION_JSON_2["state"], "alias": CONNECTION_JSON_2["alias"]},
                expiry_ms=3600000
            )
            ...
            await session.insert(
                ...
                tags={"state": CONNECTION_JSON_3["state"], "alias": CONNECTION_JSON_3["alias"]},
                expiry_ms=3600000
            )
        ...
        try:
            ...
            config_new = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="new_secure_key", pool_size=5, schema_config="normalize")
            ...
            async with store.transaction() as session:
                ...
                await session.insert(category="connection", name="conn_1", value=json.dumps(CONNECTION_JSON_1), tags={"state": CONNECTION_JSON_1["state"], "alias": CONNECTION_JSON_1["alias"]}, expiry_ms=3600000)
                await session.insert(category="connection", name="conn_2", value=json.dumps(CONNECTION_JSON_2), tags={"state": CONNECTION_JSON_2["state"], "alias": CONNECTION_JSON_2["alias"]}, expiry_ms=3600000)
                await session.insert(category="connection", name="conn_3", value=json.dumps(CONNECTION_JSON_3), tags={"state": CONNECTION_JSON_3["state"], "alias": CONNECTION_JSON_3["alias"]}, expiry_ms=3600000)
        ...
        os.makedirs(os.path.dirname(non_enc_db_path), exist_ok=True) if os.path.dirname(non_enc_db_path) else None

        print(f"Provisioning non-encrypted database at {non_enc_db_path}...")
        non_enc_config = SqliteConfig(uri=f"sqlite://{non_enc_db_path}", encryption_key=None, pool_size=5, schema_config="normalize")
        ...
        try:
            ...
            non_enc_store = SqliteDatabase(pool, profile_name, path, effective_release_number)
            print(f"Non-encrypted database ready! Profile name: {await non_enc_store.get_profile_name()}")
        except Exception as e:
        ...
        async with non_enc_store.transaction() as session:
            await session.insert(category="connection", name="conn_4", value=json.dumps(CONNECTION_JSON_1), tags={"state": CONNECTION_JSON_1["state"], "alias": CONNECTION_JSON_1["alias"]}, expiry_ms=3600000)
            await session.insert(category="connection", name="conn_5", value=json.dumps(CONNECTION_JSON_2), tags={"state": CONNECTION_JSON_2["state"], "alias": CONNECTION_JSON_2["alias"]}, expiry_ms=3600000)
            await session.insert(category="connection", name="conn_6", value=json.dumps(CONNECTION_JSON_3), tags={"state": CONNECTION_JSON_3["state"], "alias": CONNECTION_JSON_3["alias"]}, expiry_ms=3600000)
        ...
        await run_tests(non_enc_store, non_enc_db_path, non_enc_config, is_encrypted=False)

        print("\nTrying to open non-encrypted database with a key (should fail)...")
        config_with_key = SqliteConfig(uri=f"sqlite://{non_enc_db_path}", encryption_key="some_key", pool_size=5, schema_config="normalize")
        print(f"Pool size configured for non-encrypted with key: {config_with_key.pool_size}")
        try:
            pool, profile_name, path, effective_release_number = config_with_key.provision(
                profile="test_profile",
                recreate=False,
                release_number="release_0_1"
            )
            store_with_key = SqliteDatabase(pool, profile_name, path, effective_release_number)
