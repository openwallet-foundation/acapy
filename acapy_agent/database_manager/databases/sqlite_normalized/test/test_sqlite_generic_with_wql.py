...
async def run_tests(store, db_path, is_encrypted=True):
    try:
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="people", tag_filter=tag_filter, limit=1))
        print(f"Scanned with limit=1: {len(scanned_entries)} entries")
        assert len(scanned_entries) == 1, "Expected 1 entry with limit=1"
        assert scanned_entries[0].name == expected_first_person, f"Expected {expected_first_person}, got {scanned_entries[0].name}"
        try:
            value = json.loads(scanned_entries[0].value)
            print(f" - {scanned_entries[0].name}: {value}")
        except json.JSONDecodeError as e:
            print(f"Failed to parse JSON for {scanned_entries[0].name}: {scanned_entries[0].value}")
            raise
        ...
        scanned_entries_complex = list(store.scan(profile="test_profile", category="people", tag_filter=complex_scan_filter, limit=2))
        print(f"Scanned with limit=2: {len(scanned_entries_complex)} entries")
        assert len(scanned_entries_complex) == (2 if is_encrypted else 0), f"Expected {2 if is_encrypted else 0} active females (Alice, Charlie if encrypted; none if non-encrypted)"
        ...
        scanned_entries_not_male = list(store.scan(profile="test_profile", category="people", tag_filter=not_male_filter))
        print(f"Scanned not male: {len(scanned_entries_not_male)} entries")
        assert len(scanned_entries_not_male) == (2 if is_encrypted else 2), "Expected 2 not male (Alice, Charlie if encrypted; Eve, Frank if non-encrypted)"
        ...
        async with store.session() as session:
            ...
            entry = await session.fetch(category="people", name=expected_first_person, tag_filter=json.dumps({"attr::person.status": "active"}))
            assert entry is not None, f"Should fetch {expected_first_person} with status='active'"
            try:
                value = json.loads(entry.value)
                print(f"Fetched: {entry.name} with status={entry.tags['attr::person.status']}, value={value}")
            except json.JSONDecodeError as e:
                print(f"Failed to parse JSON for {entry.name}: {entry.value}")
                raise
            ...
            entry = await session.fetch(category="people", name=expected_first_person, tag_filter=json.dumps({"attr::person.status": "inactive"}))
            assert entry is None, f"Should not fetch {expected_first_person} with status='inactive'"

            # Test fetch with complex WQL: Active and female
            print(f"Fetching {'person1' if is_encrypted else 'person5'} with status='active' and gender='F'...")
            ...
            entry = await session.fetch(category="people", name="person1" if is_encrypted else "person5", tag_filter=complex_filter)
            assert entry is not None if is_encrypted else entry is None, f"Should {'fetch Alice' if is_encrypted else 'not fetch Eve'} with status='active' and gender='F'"
            if entry:
                try:
                    value = json.loads(entry.value)
                    print(f"Fetched: {entry.name} with status={entry.tags['attr::person.status']} and gender={entry.tags['attr::person.gender']}, value={value}")
                except json.JSONDecodeError as e:
                    print(f"Failed to parse JSON for {entry.name}: {entry.value}")
                    raise
        ...
        async with store.session() as session:
            ...
            entries = await session.fetch_all(category="people", tag_filter=active_females_filter)
            print(f"Found {len(entries)} active females")
            assert len(entries) == (2 if is_encrypted else 0), f"Expected {2 if is_encrypted else 0} active females (Alice, Charlie if encrypted; none if non-encrypted)"
            for entry in entries:
                try:
                    value = json.loads(entry.value)
                    print(f" - {entry.name}: {value}")
            ...
            entries = await session.fetch_all(category="people", tag_filter=pending_filter)
        ...
        async with store.transaction() as session:
            ...
            await session.replace(
                ...
                value=json.dumps({"name": "Alice Updated" if is_encrypted else "David Updated"}),
                tags={
                    "attr::person.gender": "F" if is_encrypted else "M",
                    "attr::person.status": "inactive",
                    "attr::person.birthdate::value": "19950615" if is_encrypted else "19800101"
                }
            )
            updated_entry = await session.fetch(category="people", name="person1" if is_encrypted else "person4")
            try:
                value = json.loads(updated_entry.value)
                print(f"Updated {'Alice' if is_encrypted else 'David'}: {updated_entry.name}, value={value}")
            except json.JSONDecodeError as e:
                print(f"Failed to parse JSON for {updated_entry.name}: {updated_entry.value}")
                raise
            expected_value = json.dumps({"name": "Alice Updated" if is_encrypted else "David Updated"})
            assert updated_entry.value == expected_value, "Value not updated"
            assert updated_entry.tags["attr::person.status"] == "inactive", "Tag not updated"
            assert updated_entry.tags["attr::person.birthdate::value"] == ("19950615" if is_encrypted else "19800101"), "Birthdate tag not updated"
            ...
            await session.insert(
                ...
                tags={
                    ...
                    "attr::person.status": "active",
                    "attr::person.birthdate::value": "19800101" if is_encrypted else "20010101"
                }
            )
            new_entry = await session.fetch(category="people", name="person4" if is_encrypted else "person7")
            try:
                value = json.loads(new_entry.value)
                print(f"Inserted {'David' if is_encrypted else 'Grace'}: {new_entry.name}, value={value}")
            except json.JSONDecodeError as e:
            ...
            await session.replace(
                category="people",
                name="person4" if is_encrypted else "person7",
                value=json.dumps({"name": "David Updated" if is_encrypted else "Grace Updated"}),
                tags={
                    "attr::person.gender": "M" if is_encrypted else "F",
                    "attr::person.status": "inactive",
                    "attr::person.birthdate::value": "19800101" if is_encrypted else "20010101"
            ...
            updated_entry = await session.fetch(category="people", name="person4" if is_encrypted else "person7")
            try:
                value = json.loads(updated_entry.value)
                print(f"Updated {'David' if is_encrypted else 'Grace'}: {updated_entry.name}, value={value}")
            except json.JSONDecodeError as e:
                print(f"Failed to parse JSON for {updated_entry.name}: {updated_entry.value}")
                raise
            expected_value = json.dumps({"name": "David Updated" if is_encrypted else "Grace Updated"})
            assert updated_entry.value == expected_value, "Value not updated"
            assert updated_entry.tags["attr::person.status"] == "inactive", "Tag not updated"
            assert updated_entry.tags["attr::person.birthdate::value"] == ("19800101" if is_encrypted else "20010101"), "Birthdate tag not updated"
        ...
        async with store.session() as session:
            ...
            count_inactive_males = await session.count(category="people", tag_filter=inactive_males_filter)
            print(f"Counted {count_inactive_males} inactive males")
            assert count_inactive_males == (2 if is_encrypted else 1), f"Expected {2 if is_encrypted else 1} inactive males (Bob, David if encrypted; David if non-encrypted), got {count_inactive_males}"
            if count_inactive_males > 0:
                entries = await session.fetch_all(category="people", tag_filter=inactive_males_filter)
                parsed_entries = []
                for entry in entries:
                    try:
                        value = json.loads(entry.value)
        ...
        async with store.transaction() as session:
            ...
            deleted_count = await session.remove_all(category="people", tag_filter=remove_filter)
            print(f"Deleted {deleted_count} inactive people born after 2000")
            assert deleted_count == (1 if is_encrypted else 2), f"Expected to delete {1 if is_encrypted else 2} person (Bob if encrypted; Eve, Grace if non-encrypted), got {deleted_count}"
            entries = await session.fetch_all(category="people")
            parsed_entries = []
            for entry in entries:
                try:
...
async def main():
    ...
    try:
        ...
        config = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="strong_key", pool_size=5, schema_config="generic")
        try:
        ...
        try:
            ...
            config_new = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="new_secure_key", pool_size=5, schema_config="generic")
        ...
        config_old = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="strong_key", pool_size=5, schema_config="generic")
        try:
        ...
        config_wrong = SqliteConfig(uri=f"sqlite://{db_path}", encryption_key="wrong_key", pool_size=5, schema_config="generic")
        try:
            ...
            store_wrong = SqliteDatabase(pool, profile_name, path, effective_release_number)
        ...
        non_enc_config = SqliteConfig(uri=f"sqlite://{non_enc_db_path}", encryption_key=None, pool_size=5, schema_config="generic")
        try:
        ...
        config_with_key = SqliteConfig(uri=f"sqlite://{non_enc_db_path}", encryption_key="some_key", pool_size=5, schema_config="generic")
        try:
