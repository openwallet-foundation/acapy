...
async def run_tests(store, db_path, config_new, is_encrypted=True):
    try:
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="connection", tag_filter=None, limit=1))
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="connection", tag_filter=None, offset=1))
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="connection", tag_filter=None, offset=0, limit=2))
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="connection", tag_filter=None, offset=1, limit=1))
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="connection", tag_filter=None, offset=2))
        ...
        async with store.session() as session:
            ...
            print("Removing Connection 3..." if is_encrypted else "Removing Connection 6...")
            await session.remove(category="connection", name="conn_3" if is_encrypted else "conn_6")
            removed_entry = await session.fetch(category="connection", name="conn_3" if is_encrypted else "conn_6")
            assert removed_entry is None, f"Connection {'3' if is_encrypted else '6'} should be removed"

        # Step 6: Test WQL $exist query in database
        print("\n### Testing WQL $exist Query in Database ###")
        async with store.transaction() as session:
            print("Inserting test data for $exist query...")
            await session.insert(category="connection_test", name="conn_test1", value=json.dumps({"field": "value"}), tags={})
            await session.insert(category="connection_test", name="conn_test2", value=json.dumps({}), tags={})
            await session.insert(category="connection_test", name="conn_test3", value=json.dumps({"field": "another"}), tags={})
            ...
            filtered_entries = [entry for entry in all_entries if "field" in json.loads(entry.value)]
...
async def main():
    register_backends()
    print("Starting the SQLite database test program for 'connection' category (Asyncio Version)...")
    store = None
    non_enc_store = None
    store_old = None
    store_with_key = None
    try:
        ...
        try:
            ...
            async with store.transaction() as session:
                ...
                await session.insert(category="connection", name="conn_1", value=json.dumps(CONNECTION_JSON_1), tags={}, expiry_ms=3600000)
                await session.insert(category="connection", name="conn_2", value=json.dumps(CONNECTION_JSON_2), tags={}, expiry_ms=3600000)
                await session.insert(category="connection", name="conn_3", value=json.dumps(CONNECTION_JSON_3), tags={}, expiry_ms=3600000)
        ...
        async with non_enc_store.transaction() as session:
            await session.insert(category="connection", name="conn_4", value=json.dumps(CONNECTION_JSON_1), tags={})
            await session.insert(category="connection", name="conn_5", value=json.dumps(CONNECTION_JSON_2), tags={})
            await session.insert(category="connection", name="conn_6", value=json.dumps(CONNECTION_JSON_3), tags={})
