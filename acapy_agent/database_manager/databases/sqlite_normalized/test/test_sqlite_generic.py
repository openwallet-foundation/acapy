...
async def run_tests(store, db_path, is_encrypted=True):
    try:
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="people", tag_filter=tag_filter, offset=1))
        print(f"Scanned with offset=1: {len(scanned_entries)} entries")
        assert len(scanned_entries) == 1, "Expected 1 entry with offset=1"
        assert scanned_entries[0].name == expected_second_person, f"Expected {expected_second_person}, got {scanned_entries[0].name}"
        try:
            value = json.loads(scanned_entries[0].value)
            print(f" - {scanned_entries[0].name}: {value}")
        except json.JSONDecodeError as e:
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="people", tag_filter=tag_filter, offset=0, limit=2))
        ...
        assert scanned_entries[0].name == expected_first_person and scanned_entries[1].name == expected_second_person, f"Expected {expected_first_person} and {expected_second_person}"
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="people", tag_filter=tag_filter, offset=1, limit=1))
        ...
        scanned_entries = list(store.scan(profile="test_profile", category="people", tag_filter=tag_filter, offset=2))
        ...
        async with store.session() as session:
            ...
            assert len(entries) == (3 if is_encrypted else 2), f"Expected {3 if is_encrypted else 2} people after deletion"
        ...
        async with store.transaction() as session:
            ...
            wql_query = json.dumps({
                "$and": [
                    ...
                    {
                        "attr::person.name.family::value": {"$in": ["DOE22sss", "SMITH", "JOHNSON"]}
                    },
                    {
            ...
            scanned_entries = await session.fetch_all(category="credential", tag_filter=wql_query, limit=10)
        ...
        async with store.transaction() as session:
            ...
            print(f"Wiped out {deleted_count_people} people and {deleted_count_credentials} credentials!")
            assert deleted_count_people == (3 if is_encrypted else 2), f"Expected to delete {3 if is_encrypted else 2} people after deletion"
...
async def main():
    ...
    try:
        ...
        for db_store in [store, non_enc_store, store_old, store_wrong, store_with_key]:
            if db_store is not None and hasattr(db_store, 'pool') and db_store.pool is not None:
                try:
