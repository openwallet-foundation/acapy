...
class SqliteConfig:
    ...
    def provision(self, profile: Optional[str] = None, recreate: bool = False, release_number: str = "release_0") -> Tuple[ConnectionPool, str, str, str]:
        ...
        effective_release_number = "release_0" if self.schema_config == "generic" else release_number
        ...
        try:
            ...
            if effective_release_number != "release_0":
                LOGGER.debug("Loading schema release: %s (type: sqlite)", effective_release_number)
                ...
                for category, schema in schemas.items():
                    ...
                    if "sqlite" not in schema:
                        LOGGER.warning("Skipping category %s: no sqlite schema found in %s", category, schema)
                        continue

                    LOGGER.debug("Applying SQLite schema for category %s: %s", category, schema["sqlite"])
                    for idx, sql in enumerate(schema["sqlite"]):
                        LOGGER.debug("Executing SQL [%d] for category '%s': %s", idx + 1, category, sql.strip())
                        try:
                            cursor.execute(sql)
                            LOGGER.debug("Successfully executed SQL [%d] for category '%s'", idx + 1, category)
                        except sqlite3.OperationalError as e:
                            LOGGER.error(
                                "Failed to apply schema for category '%s' at statement [%d]: %s\nSQL: %s",
                                category, idx + 1, str(e), sql.strip()
                            )
                            raise DatabaseError(
            ...
            cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS ix_items_profile_category_name ON items (profile_id, category, name)")
            cursor.execute("CREATE INDEX IF NOT EXISTS ix_items_tags_item_id ON items_tags (item_id)")
            cursor.execute("CREATE INDEX IF NOT EXISTS ix_items_expiry ON items (expiry)")
            default_profile = profile or "default_profile"
            cursor.execute("INSERT OR IGNORE INTO config (name, value) VALUES ('default_profile', ?)", (default_profile,))
            cursor.execute("INSERT OR IGNORE INTO config (name, value) VALUES ('key', NULL)")
            cursor.execute("INSERT OR IGNORE INTO config (name, value) VALUES ('schema_release_number', ?)", (effective_release_number,))
            cursor.execute("INSERT OR IGNORE INTO config (name, value) VALUES ('schema_release_type', 'sqlite')")
            cursor.execute("INSERT OR IGNORE INTO config (name, value) VALUES ('schema_config', ?)", (self.schema_config,))
            cursor.execute("INSERT OR IGNORE INTO profiles (name, profile_key) VALUES (?, NULL)", (default_profile,))
            cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS ix_profile_name ON profiles (name);")
            conn.commit()
        except Exception as e:
            conn.rollback()
    ...
    def _apply_migrations(self, conn, current_release: str, target_release: str, db_type: str = "sqlite"):
        ...
        for i in range(current_index, target_index):
            ...
            try:
                ...
                LOGGER.error(f"{db_type} migration failed from {from_release} to {to_release}: {str(e)}")
                raise DatabaseError(
                    code=DatabaseErrorCode.PROVISION_ERROR,
    ...
    def open(self, profile: Optional[str] = None, schema_migration: Optional[bool] = None, target_schema_release_number: Optional[str] = None) -> Tuple[ConnectionPool, str, str, str]:
            ...
            try:
                cursor = conn.cursor()
                cursor.execute("SELECT value FROM config WHERE name = 'schema_release_number'")
                ...
                if self.schema_config == "generic" and db_current_release != "release_0":
                    LOGGER.error("Invalid configuration: schema_config='generic' but schema_release_number='%s'", db_current_release)
                    raise DatabaseError(
                ...
                if self.schema_config == "normalize" and target_schema_release_number and db_current_release != target_schema_release_number:
                    LOGGER.error("Schema release number mismatch: database has '%s', but target is '%s'", db_current_release, target_schema_release_number)
                    raise DatabaseError(
                        code=DatabaseErrorCode.UNSUPPORTED_VERSION,
