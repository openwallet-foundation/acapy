...
import asyncio
import time
from typing import Optional, Union, AsyncGenerator, Sequence
import urllib.parse

from psycopg_pool import AsyncConnectionPool
from psycopg import AsyncConnection, AsyncCursor, pq
...
class PostgresDatabase(AbstractDatabaseStore):
    def __init__(self, pool: AsyncConnectionPool, default_profile: str, conn_str: str, release_number: str = "release_0",
                 max_sessions: Optional[int] = None, min_size: Optional[int] = None, max_size: Optional[int] = None,
                 timeout: Optional[float] = None, max_idle: Optional[float] = None, max_lifetime: Optional[float] = None,
                 schema_context: Optional[SchemaContext] = None, backend: Optional["PostgresqlBackend"] = None):
        ...
        self.max_sessions = max_sessions if max_sessions is not None else int(pool.max_size * 0.75)
        ...
        self.max_lifetime = max_lifetime if max_lifetime is not None else pool.max_lifetime
        self.schema_context = schema_context or SchemaContext()  # Default to SchemaContext
    ...
    async def initialize(self):
        try:
            ...
            LOGGER.error("Failed to initialize default profile ID for '%s': %s", self.default_profile, str(e))
            raise DatabaseError(
    ...
    async def create_profile(self, name: str = None) -> str:
        ...
        try:
            async with conn.cursor() as cursor:
                await cursor.execute(f"INSERT INTO {self.schema_context.qualify_table('profiles')} (name, profile_key) VALUES (%s, NULL) ON CONFLICT (name) DO NOTHING", (name,))
    ...
    async def remove_profile(self, name: str) -> bool:
        ...
        try:
            async with conn.cursor() as cursor:
                await cursor.execute(f"DELETE FROM {self.schema_context.qualify_table('profiles')} WHERE name = %s", (name,))
    ...
    async def scan(self, profile: Optional[str], category: str, tag_filter: Union[str, dict] = None,
                   offset: int = None, limit: int = None, order_by: Optional[str] = None, descending: bool = False) -> AsyncGenerator[Entry, None]:
        ...
        try:
            async with conn.cursor() as cursor:
                async for entry in handler.scan(cursor, profile_id, category, tag_query, offset, limit, order_by, descending):
                    yield entry
    ...
    async def scan_keyset(self, profile: Optional[str], category: str, tag_filter: Union[str, dict] = None,
                          last_id: Optional[int] = None, limit: int = None, order_by: Optional[str] = None, descending: bool = False) -> AsyncGenerator[Entry, None]:
        ...
        try:
            async with conn.cursor() as cursor:
                async for entry in handler.scan_keyset(cursor, profile_id, category, tag_query, last_id, limit, order_by, descending):
                    yield entry
                if conn.pgconn.transaction_status != pq.TransactionStatus.IDLE:
                    await conn.commit()
        except Exception as e:
    ...
    async def session(self, profile: str = None) -> 'PostgresSession':
        ...
        sess = PostgresSession(self, profile or self.default_profile, False, self.release_number)
        with self.lock:
    ...
    async def transaction(self, profile: str = None) -> 'PostgresSession':
        ...
        sess = PostgresSession(self, profile or self.default_profile, True, self.release_number)
        with self.lock:
            self.active_sessions.append(sess)
            self.session_creation_times[id(sess)] = time.time()
        LOGGER.debug("[session] Active sessions: %d, session_id=%s", len(self.active_sessions), id(sess))
            ...
            if remove:
                ...
                try:
                    ...
                    try:
                        ...
                        async with conn.cursor() as cursor:
                            await cursor.execute(f"SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = %s AND pid <> pg_backend_pid()", (target_db,))
