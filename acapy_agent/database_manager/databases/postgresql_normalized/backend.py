...
class PostgresqlBackend:
    async def provision(
        ...
        min_size = int(config.get("min_connections", query_params.get("min_connections", [4])[0]))
        max_size = int(config.get("max_connections", query_params.get("max_connections", [10])[0]))
        timeout = float(config.get("connect_timeout", query_params.get("connect_timeout", [30.0])[0]))
        max_idle = float(config.get("max_idle", query_params.get("max_idle", [5.0])[0]))
        max_lifetime = float(config.get("max_lifetime", query_params.get("max_lifetime", [3600.0])[0]))
        max_sessions = int(config.get("max_sessions", query_params.get("max_sessions", [None])[0] or max_size * 0.75))
        ...
        pool, profile_name, conn_str, effective_release_number = await config_obj.provision(
            profile=profile,
            recreate=recreate,
    ...
    def translate_error(self, error: Exception) -> DBStoreError:
        ...
        if isinstance(error, DatabaseError):
            ...
            if error.code == DatabaseErrorCode.PROFILE_NOT_FOUND:
                return DBStoreError(
                    code=DBStoreErrorCode.NOT_FOUND,
                    message="Database profile not found"
                )
            ...
            if error.code == DatabaseErrorCode.DUPLICATE_ITEM_ENTRY_ERROR:
                return DBStoreError(
                    code=DBStoreErrorCode.DUPLICATE,
                    message="Duplicate Item Entry Error"
                )
