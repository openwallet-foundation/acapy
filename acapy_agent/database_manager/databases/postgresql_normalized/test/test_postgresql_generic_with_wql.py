...
from acapy_agent.database_manager.databases.postgresql_normalized.config import PostgresConfig
    ...
    try:
        async with await store.transaction(profile="test_profile") as session:
            ...
            await session.insert(
                ...
                tags={"attr::person.status": "active", "attr::person.gender": "M", "attr::person.birthdate::value": "19800101"},
            )
            print("Adding Eve...")
            await session.insert(
                category="people",
                ...
                tags={"attr::person.status": "inactive", "attr::person.gender": "F", "attr::person.birthdate::value": "20010101"},
            )
            print("Adding Frank...")
            await session.insert(
                category="people",
                ...
                tags={"attr::person.status": "active", "attr::person.gender": "O", "attr::person.birthdate::value": "19950101"},
            )
...
async def run_tests(store, conn_str):
    ...
    async for entry in store.scan(profile="test_profile", category="people", tag_filter={"attr::person.status": "active"}, limit=1):
        try:
            value = json.loads(entry.value)
    ...
    async for entry in store.scan(profile="test_profile", category="people", tag_filter={"$and": [{"attr::person.status": "active"}, {"attr::person.gender": "F"}]}, limit=2):
        try:
            value = json.loads(entry.value)
    ...
    async for entry in store.scan(profile="test_profile", category="people", tag_filter={"$not": {"attr::person.gender": "M"}}):
        try:
            value = json.loads(entry.value)
    ...
    async with await store.session(profile="test_profile") as session:
        ...
        entry = await session.fetch(category="people", name="person4", tag_filter={"attr::person.status": "active"})
        if entry:
            try:
                value = json.loads(entry.value)
        ...
        entry = await session.fetch(category="people", name="person4", tag_filter={"attr::person.status": "inactive"})
        if not entry:
            print("No person4 with status=inactive")
        print("Fetching person5 with status='active' and gender='F'...")
        entry = await session.fetch(category="people", name="person5", tag_filter={"$and": [{"attr::person.status": "active"}, {"attr::person.gender": "F"}]})
        if not entry:
            print("No person5 with status=active and gender=F")

    async with await store.session(profile="test_profile") as session:
        print("Fetching all active females...")
        entries = await session.fetch_all(category="people", tag_filter={"$and": [{"attr::person.status": "active"}, {"attr::person.gender": "F"}]})
        parsed_entries = []
        ...
        entries = await session.fetch_all(category="people", tag_filter={"attr::person.status": "pending"})
        parsed_entries = []
        for entry in entries:
            try:
                value = json.loads(entry.value)
    ...
    async with await store.transaction(profile="test_profile") as session:
        ...
        await session.replace(
            ...
            tags={"attr::person.status": "inactive", "attr::person.gender": "M", "attr::person.birthdate::value": "19800101"},
        )
        ...
        try:
            ...
            raise
        assert updated_entry.value == json.dumps({"name": "David Updated"}), "Value not updated"
