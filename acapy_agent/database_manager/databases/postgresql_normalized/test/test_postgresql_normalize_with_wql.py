...
import json
import logging
from acapy_agent.database_manager.databases.postgresql_normalized.backend import PostgresqlBackend
from acapy_agent.database_manager.databases.postgresql_normalized.database import PostgresDatabase
...
async def run_tests(store: PostgresDatabase, conn_str: str):
    try:
        ...
        scanned_entries = []
        async for entry in store.scan(profile="test_profile", category="connection", tag_filter=wql_equality, limit=2):
        ...
        async with session:
            ...
            entry = await session.fetch(category="connection", name="conn_1", tag_filter=json.dumps({"state": "completed"}))
            ...
            entry = await session.fetch(category="connection", name="conn_1", tag_filter=json.dumps({"state": "active"}))
            ...
            entry = await session.fetch(category="connection", name="conn_2", tag_filter=json.dumps({"$exist": ["alias"]}))
            ...
            LOGGER.debug("Executing WQL query: %s for conn_3", json.dumps({"$exist": ["alias"]}))
            entry = await session.fetch(category="connection", name="conn_3", tag_filter=json.dumps({"$exist": ["alias"]}))
            ...
            entry = await session.fetch(category="connection", name="conn_1", tag_filter=json.dumps({"created_at": {"$lt": "2025-05-08T00:00:00Z"}}))
            ...
            entry = await session.fetch(category="connection", name="conn_3", tag_filter=json.dumps({"created_at": {"$lt": "2025-05-08T00:00:00Z"}}))
            assert entry is None, "Should not fetch conn_3 with created_at < '2025-05-08T00:00:00Z'"
        ...
        transaction = await store.transaction(profile="test_profile")
        async with transaction:
            deleted_count = await transaction.remove_all(category="connection", tag_filter=wql_remove)
...
async def main():
    register_backends()
    print("=== Starting PostgreSQL Normalized Schema Test (Connection Category with WQL Queries) ===")
    store = None
    try:
        ...
        try:
            ...
            assert profile_name == "test_profile", f"Profile name mismatch: expected 'test_profile', got '{profile_name}'"
