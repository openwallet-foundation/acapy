...
class PresExV20CustomHandler(NormalizedHandler):
    ...
    def __init__(self, category: str, columns: List[str], table_name: Optional[str] = None, schema_context: Optional[SchemaContext] = None):
        ...
        LOGGER.debug(f"Initialized PresExV20CustomHandler for category={category}, table={self.table}, columns={columns}, schema_context={schema_context}")
    ...
    def _extract_revealed_attrs(self, json_data: dict) -> str:
        try:
            ...
            presentations_attach = pres.get('presentations_attach', []) or pres.get('presentations~attach', [])
            if not presentations_attach or not isinstance(presentations_attach, list):
                return json.dumps([])
            
            attrs = []
            for attachment in presentations_attach:
                if attachment.get('mime-type') == 'application/json' and attachment.get('data', {}).get('base64'):
                    data = attachment['data']['base64']
                    try:
                        decoded_data = base64.b64decode(data).decode('utf-8')
                        if is_valid_json(decoded_data):
                            decoded_json = json.loads(decoded_data)
                            revealed_attr_groups = decoded_json.get('requested_proof', {}).get('revealed_attr_groups', {})
                            for group in revealed_attr_groups.values():
                                for attr_name, attr_data in group.get('values', {}).items():
                                    if 'raw' in attr_data:
                                        attrs.append({
                                            'attr_name': attr_name,
    ...
    async def insert(self, cursor: AsyncCursor, profile_id: int, category: str, name: str, 
        ...
        LOGGER.debug(f"[insert] Inserting record with category={category}, name={name}, value={value}, tags={tags}")
        ...
        LOGGER.debug(f"[insert] Added revealed_attr_groups to json_data: {json_data['revealed_attr_groups']}")
    ...
    async def replace(self, cursor: AsyncCursor, profile_id: int, category: str, name: str, 
        ...
        json_data['revealed_attr_groups'] = self._extract_revealed_attrs(json_data)
        LOGGER.debug(f"[replace] Added revealed_attr_groups to json_data: {json_data['revealed_attr_groups']}")
