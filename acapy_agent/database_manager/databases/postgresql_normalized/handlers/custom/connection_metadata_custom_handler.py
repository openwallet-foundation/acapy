...
class ConnectionMetadataCustomHandler(NormalizedHandler):
    ...
    def __init__(self, category: str, columns: List[str], table_name: Optional[str] = None, schema_context: Optional[SchemaContext] = None):
        ...
        LOGGER.debug(f"Initialized ConnectionMetadataCustomHandler for category={category}, table={self.table}, columns={columns}, schema_context={schema_context}")
    ...
    async def insert(self, cursor: AsyncCursor, profile_id: int, category: str, name: str, 
        ...
        for col in self.columns:
            if col == 'metadata' and metadata:
                data[col] = metadata
                LOGGER.debug(f"[insert] Added column {col} from custom extraction: {metadata}")
    ...
    async def replace(self, cursor: AsyncCursor, profile_id: int, category: str, name: str, 
        ...
        for col in self.columns:
            if col == 'metadata' and metadata:
                data[col] = metadata
                LOGGER.debug(f"[replace] Added column {col} from custom extraction: {metadata}")
            elif col in json_data:
                val = json_data[col]
