...
class CredExV20CustomHandler(NormalizedHandler):
    ...
    def __init__(self, category: str, columns: List[str], table_name: Optional[str] = None, schema_context: Optional[SchemaContext] = None):
        ...
        LOGGER.debug(f"Initialized CredExV20CustomHandler for category={category}, table={self.table}, columns={columns}, version={self.version}, schema_context={schema_context}")

    def _get_version(self) -> str:
        try:
            table_suffix = self.table[len(f"{self.schema_context}.cred_ex_v20_v"):]
            if table_suffix:
                LOGGER.debug(f"Extracted version {table_suffix} from table name {self.table}")
                return table_suffix
    ...
    async def _extract_attributes_and_formats(self, json_data: dict, cred_ex_id: int, cursor: AsyncCursor):
        ...
        attributes_table = self.schema_context.qualify_table(f"cred_ex_v20_attributes_v{self.version}")
        try:
            await cursor.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = %s AND table_schema = %s)", 
            ...
            LOGGER.error(f"[extract] Database error inserting into {attributes_table}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
        ...
        formats_table = self.schema_context.qualify_table(f"cred_ex_v20_formats_v{self.version}")
        try:
            await cursor.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = %s AND table_schema = %s)", 
            ...
            LOGGER.error(f"[extract] Database error inserting into {formats_table}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def insert(self, cursor: AsyncCursor, profile_id: int, category: str, name: str, 
        ...
        LOGGER.debug(f"[insert] Starting with category={category}, name={name}, value={value}, tags={tags}")
        ...
        try:
            ...
            if existing_item:
                ...
                await cursor.execute(f"SELECT id, thread_id FROM {self.table} WHERE item_id = %s", (item_id,))
            ...
            if tags.get('thread_id'):
                await cursor.execute(f"SELECT id, item_id FROM {self.table} WHERE thread_id = %s", (tags.get('thread_id'),))
            ...
            LOGGER.error(f"[insert] Database error during insert for item_id={item_id}, thread_id={tags.get('thread_id')}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def replace(self, cursor: AsyncCursor, profile_id: int, category: str, name: str, 
                      value: Union[str, bytes, dict], tags: dict, expiry_ms: int) -> None:
        LOGGER.debug(f"[replace] Starting with category={category}, name={name}, thread_id={tags.get('thread_id')}")
        ...
        try:
            ...
            if tags.get('thread_id'):
                await cursor.execute(f"SELECT id, item_id FROM {self.table} WHERE thread_id = %s AND item_id != %s", 
                ...
                if duplicates:
                    ...
                    for dup_id, dup_item_id in duplicates:
                        await cursor.execute(f"DELETE FROM {self.table} WHERE id = %s", (dup_id,))
            ...
            LOGGER.error(f"[replace] Database error during replace for item_id={item_id}, thread_id={tags.get('thread_id')}: {str(e)}")
            raise DatabaseError(
                code=DatabaseErrorCode.QUERY_ERROR,
