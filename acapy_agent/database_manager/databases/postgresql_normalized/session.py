...
from psycopg import AsyncConnection, AsyncCursor, pq, errors as psycopg_errors
...
class PostgresSession(AbstractDatabaseSession):
    def __init__(self, database: PostgresDatabase, profile: str, is_txn: bool, release_number: str = "release_1"):
    ...
    def _process_value(self, value: Union[str, bytes], operation: str, name: str, category: str) -> str:
        """Process items.value for insert/replace (encode) or fetch/fetch_all (decode)."""
        if operation in ("insert", "replace"):
            if isinstance(value, bytes):
                try:
                    processed_value = value.decode('utf-8')
                    LOGGER.debug("Converted bytes to UTF-8 string for %s in category %s: %s", name, category, processed_value)
                    return processed_value
                except UnicodeDecodeError as e:
                    LOGGER.error("Failed to decode bytes value for %s in category %s: %s", name, category, str(e))
                    raise DatabaseError(
            ...
            if isinstance(value, str) and value.startswith('\\x'):
                try:
                    ...
                    LOGGER.debug("Decoded hex value for %s in category %s: %s", name, category, processed_value)
                    return processed_value
                except (binascii.Error, UnicodeDecodeError) as e:
                    LOGGER.error("Failed to decode hex-encoded value for %s in category %s: %s", name, category, str(e))
                    raise DatabaseError(
                        code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def __aenter__(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                LOGGER.debug("[enter_session] Starting for profile=%s, is_txn=%s, release_number=%s", self.profile, self.is_txn, self.release_number)
                return self
                ...
                if attempt < max_retries - 1:
                    ...
                    continue
                LOGGER.error("Failed to enter session after %d retries: %s", max_retries, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.CONNECTION_ERROR,
    ...
    async def count(self, category: str, tag_filter: Union[str, dict] = None) -> int:
        ...
        async with self.conn.cursor() as cursor:
            try:
                ...
                if not self.is_txn and self.conn.pgconn.transaction_status != pq.TransactionStatus.IDLE:
                    await self.conn.commit()
                return count
            except asyncio.CancelledError:
                if not self.is_txn:
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to count items in category '%s': %s", category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def insert(self, category: str, name: str, value: Union[str, bytes] = None, 
                     tags: dict = None, expiry_ms: int = None):
        handlers, _, _ = get_release(self.release_number, "postgresql")
        handler = handlers.get(category, handlers["default"])
        value = self._process_value(value, "insert", name, category)
        async with self.conn.cursor() as cursor:
            try:
                await handler.insert(cursor, self.profile_id, category, name, value, tags or {}, expiry_ms)
                if not self.is_txn and self.conn.pgconn.transaction_status != pq.TransactionStatus.IDLE:
                    await self.conn.commit()
            except asyncio.CancelledError:
                if not self.is_txn:
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to insert item '%s' in category '%s': %s", name, category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def fetch(self, category: str, name: str, tag_filter: Union[str, dict] = None, 
                    for_update: bool = False) -> Optional[Entry]:
        handlers, _, _ = get_release(self.release_number, "postgresql")
        handler = handlers.get(category, handlers["default"])
        async with self.conn.cursor() as cursor:
            try:
                result = await handler.fetch(cursor, self.profile_id, category, name, tag_filter, for_update)
                if result:
                    result = Entry(
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to fetch item '%s' in category '%s': %s", name, category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def fetch_all(self, category: str, tag_filter: Union[str, dict] = None, 
                        limit: int = None, for_update: bool = False,
                        order_by: Optional[str] = None, descending: bool = False) -> Sequence[Entry]:
        handlers, _, _ = get_release(self.release_number, "postgresql")
        handler = handlers.get(category, handlers["default"])
        async with self.conn.cursor() as cursor:
            try:
                results = await handler.fetch_all(cursor, self.profile_id, category, tag_filter, limit, for_update, order_by, descending)
                decoded_results = [
                    Entry(
                        category=result.category,
                        name=result.name,
                        value=self._process_value(result.value, "fetch_all", result.name, category),
                        tags=result.tags
                    )
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to fetch all items in category '%s': %s", category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def replace(self, category: str, name: str, value: Union[str, bytes] = None, 
        ...
        async with self.conn.cursor() as cursor:
            try:
                await handler.replace(cursor, self.profile_id, category, name, value, tags or {}, expiry_ms)
                if not self.is_txn and self.conn.pgconn.transaction_status != pq.TransactionStatus.IDLE:
                    await self.conn.commit()
            except asyncio.CancelledError:
                if not self.is_txn:
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to replace item '%s' in category '%s': %s", name, category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def remove(self, category: str, name: str):
        ...
        async with self.conn.cursor() as cursor:
            try:
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to remove item '%s' in category '%s': %s", name, category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
    ...
    async def remove_all(self, category: str, tag_filter: Union[str, dict] = None) -> int:
        ...
        async with self.conn.cursor() as cursor:
            try:
                result = await handler.remove_all(cursor, self.profile_id, category, tag_filter)
                if not self.is_txn and self.conn.pgconn.transaction_status != pq.TransactionStatus.IDLE:
                    await self.conn.commit()
                return result
            except asyncio.CancelledError:
                ...
                if not self.is_txn:
                    await self.conn.rollback()
                LOGGER.error("Failed to remove all items in category '%s': %s", category, str(e))
                raise DatabaseError(
                    code=DatabaseErrorCode.QUERY_ERROR,
