...
class SchemaContext:
    ...
    def __init__(self, schema_name: Optional[str] = None):
        self.schema_name = self._validate_schema_name(schema_name or self.DEFAULT_SCHEMA_NAME)

    def _validate_schema_name(self, schema_name: str) -> str:
        if not re.match(r'^[a-zA-Z0-9_]+$', schema_name):
            raise ValueError(f"Invalid schema name '{schema_name}': must contain only alphanumeric characters and underscores")
        return schema_name
