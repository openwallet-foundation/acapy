...
def load_schema(category: str, version: str) -> dict:
    ...
    try:
        ...
        LOGGER.error("Failed to load schema for category=%s, version=%s: %s", category, version, str(e))
        return {"schemas": {}, "columns": [], "drop_schemas": {}}
    except Exception as e:
        LOGGER.error("Unexpected error loading schema for category=%s, version=%s: %s", category, version, str(e))
        return {"schemas": {}, "columns": [], "drop_schemas": {}}
...
def load_release(release_number: str) -> dict:
    ...
    try:
        ...
        LOGGER.error("Unexpected error loading release module %s: %s", release_number, str(e))
        raise ValueError(f"Unexpected error loading release module {release_number}: {str(e)}")
...
def get_release(release_number: str, db_type: str = "sqlite") -> Tuple[dict, dict, dict]:
    ...
    if release_number not in RELEASE_ORDER:
        LOGGER.error("Invalid release number: %s, expected one of %s", release_number, RELEASE_ORDER)
    ...
    if release_number == "release_0":
        ...
        if not default_handler:
            LOGGER.error("Database type %s not supported for default handler in release %s", db_type, release_number)
            raise ValueError(f"Database type {db_type} not supported for default handler in release {release_number}")
        for category in release:
            handlers[category] = default_handler() if callable(default_handler) else default_handler
        ...
        for category, info in release.items():
            if category == "default" and not info["schemas"]:
                ...
                if db_type not in info["handlers"]:
                    LOGGER.error("Database type %s not supported for category %s in release %s", db_type, category, release_number)
                    raise ValueError(f"Database type {db_type} not supported for category {category} in release {release_number}")
                ...
                drop_list = info["drop_schemas"].get(db_type) if info.get("drop_schemas") else None 
                drop_schemas[category] = {db_type: drop_list} if drop_list else None
                if schemas[category] is None and info["schemas"]:
                    LOGGER.warning("No schema found for category=%s, db_type=%s, despite schemas being defined: %s", category, db_type, info["schemas"])
