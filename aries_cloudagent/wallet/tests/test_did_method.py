...
from ..key_type import BLS12381G1, BLS12381G1G2, BLS12381G2, ED25519, X25519, KeyType
...
class TestKeyType(TestCase):
    def test_from_multicodec_name(self):
        ...
        assert KeyType.from_multicodec_name(ED25519_MULTICODEC_NAME) == ED25519
        assert KeyType.from_multicodec_name(X25519_MULTICODEC_NAME) == X25519
        assert (
    ...
    def test_from_key_type(self):
        ...
        assert KeyType.from_key_type(ED25519_KEY_NAME) == ED25519
        assert KeyType.from_key_type(X25519_KEY_NAME) == X25519
        assert KeyType.from_key_type(BLS12381G1_KEY_NAME) == BLS12381G1
        assert KeyType.from_key_type(BLS12381G2_KEY_NAME) == BLS12381G2
        assert KeyType.from_key_type(BLS12381G1G2_KEY_NAME) == BLS12381G1G2
        assert KeyType.from_key_type("non-existing") == None

    def test_from_multicodec_prefix(self):

        assert KeyType.from_multicodec_prefix(ED25519_PREFIX_BYTES) == ED25519
        assert KeyType.from_multicodec_prefix(X25519_PREFIX_BYTES) == X25519
        assert (
