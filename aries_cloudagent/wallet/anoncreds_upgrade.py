...
async def retry_converting_records(
    ...
    try:
        ...
        await set_storage_type_and_update_profile_if_subwallet(profile, is_subwallet)
...
async def upgrade_wallet_to_anoncreds(profile: Profile, is_subwallet=False) -> None:
...
async def set_storage_type_to_anoncreds(profile: Profile):
    """Set storage type to anoncreds."""
    async with profile.session() as session:
        storage = session.inject(BaseStorage)
        try:
            ...
            await storage.update_record(storage_type_record, "askar-anoncreds", {})
...
async def remove_upgrading_record(profile: Profile):
    """Remove upgrading record."""
    async with profile.session() as session:
        storage = session.inject(BaseStorage)
        try:
            upgrading_record = await storage.find_record(
...
async def set_storage_type_and_update_profile_if_subwallet(
    ...
    async with profile.session() as session:
        if is_subwallet:
            ...
            await remove_upgrading_record(profile)
