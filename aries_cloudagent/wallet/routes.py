...
class DIDSchema(OpenAPISchema):
    ...
    did = fields.Str(validate=GENERIC_DID_VALIDATE, metadata={'description':
        'DID of interest', 'example': GENERIC_DID_EXAMPLE})
    verkey = fields.Str(validate=INDY_RAW_PUBLIC_KEY_VALIDATE, metadata={
        'description': 'Public verification key', 'example':
        INDY_RAW_PUBLIC_KEY_EXAMPLE})
    posture = fields.Str(validate=DID_POSTURE_VALIDATE, metadata={'description':
    ...
    method = fields.Str(metadata={'description':
        'Did method associated with the DID', 'example': SOV.method_name})
    key_type = fields.Str(validate=validate.OneOf([ED25519.key_type, BLS12381G2
        .key_type]), metadata={'description':
        'Key type associated with the DID', 'example': ED25519.key_type})
...
class DIDListSchema(OpenAPISchema):
    ...
    results = fields.List(fields.Nested(DIDSchema()), metadata={'description':
        'DID list'})
...
class DIDEndpointWithTypeSchema(OpenAPISchema):
    ...
    endpoint = fields.Str(required=False, validate=ENDPOINT_VALIDATE, metadata=
        {'description': 'Endpoint to set (omit to delete)', 'example':
        ENDPOINT_EXAMPLE})
    endpoint_type = fields.Str(required=False, validate=ENDPOINT_VALIDATE,
        metadata={'description':
        f"Endpoint type to set (default '{EndpointType.ENDPOINT.w3c}'); affects only public or posted DIDs"
...
class JWSCreateSchema(OpenAPISchema):
    ...
    did = fields.Str(required=False, validate=GENERIC_DID_VALIDATE, metadata={
        'description': 'DID of interest', 'example': GENERIC_DID_EXAMPLE})
    verification_method = fields.Str(data_key='verificationMethod', required=
        False, validate=Uri(), metadata={'description':
        'Information used for proof verification', 'example':
...
class JWSVerifyResponseSchema(OpenAPISchema):
    ...
    headers = fields.Dict(required=True, metadata={'description':
        'Headers from verified JWT.'})
    payload = fields.Dict(required=True, metadata={'description':
...
class DIDListQueryStringSchema(OpenAPISchema):
    ...
    did = fields.Str(required=False, validate=GENERIC_DID_VALIDATE, metadata={
        'description': 'DID of interest', 'example': GENERIC_DID_EXAMPLE})
    verkey = fields.Str(required=False, validate=INDY_RAW_PUBLIC_KEY_VALIDATE,
        metadata={'description': 'Verification key of interest', 'example':
        INDY_RAW_PUBLIC_KEY_EXAMPLE})
    posture = fields.Str(required=False, validate=DID_POSTURE_VALIDATE,
        metadata={'description':
        'Whether DID is current public DID, posted to ledger but current public DID, or local to the wallet'
        , 'example': DID_POSTURE_EXAMPLE})
    method = fields.Str(required=False, validate=validate.OneOf([KEY.
        method_name, SOV.method_name]), metadata={'example': KEY.method_name,
        'description':
        'DID method to query for. e.g. sov to only fetch indy/sov DIDs'})
    key_type = fields.Str(required=False, validate=validate.OneOf([ED25519.
        key_type, BLS12381G2.key_type]), metadata={'example': ED25519.key_type,
...
class DIDQueryStringSchema(OpenAPISchema):
    ...
    did = fields.Str(required=True, validate=GENERIC_DID_VALIDATE, metadata={
        'description': 'DID of interest', 'example': GENERIC_DID_EXAMPLE})
...
class DIDCreateOptionsSchema(OpenAPISchema):
    ...
    key_type = fields.Str(required=True, validate=validate.OneOf([ED25519.
        key_type, BLS12381G2.key_type]), metadata={'example': ED25519.key_type,
    ...
    did = fields.Str(required=False, validate=GENERIC_DID_VALIDATE, metadata={
        ...
        'if the method supports or requires so.', 'example': GENERIC_DID_EXAMPLE})


class DIDCreateSchema(OpenAPISchema):
    """Parameters and validators for create DID endpoint."""

    method = fields.Str(required=False, dump_default=SOV.method_name, metadata=
        {'example': SOV.method_name, 'description': 
        'Method for the requested DID.' +
        "Supported methods are 'key', 'sov', and any other registered method."})

    options = fields.Nested(DIDCreateOptionsSchema, required=False, metadata={
        'description':
        'To define a key type and/or a did depending on chosen DID method.'})

    seed = fields.Str(required=False, metadata={'description':
...
class MediationIDSchema(OpenAPISchema):
    ...
    mediation_id = fields.Str(required=False, metadata={'description':
