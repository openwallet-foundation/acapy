...
from .models.pres_exchange import V30PresExRecord, V30PresExRecordSchema


class V30PresentProofModuleResponseSchema(OpenAPISchema):
    """Response schema for Present Proof Module."""


class V30PresExRecordListQueryStringSchema(OpenAPISchema):
...
class V30PresExRecordListSchema(OpenAPISchema):
    """Result schema for a presentation exchange query."""

    results = fields.List(
        fields.Nested(V30PresExRecordSchema()),
        description="Presentation exchange records",
    )


class V30PresProposalByFormatSchema(OpenAPISchema):
    ...
    def validate_fields(self, data, **kwargs):
        ...
        """
        if not any(f.api in data for f in V30PresFormat.Format):
            raise ValidationError(
                "V30PresProposalByFormatSchema requires indy, dif, or both"
            )


class V30PresProposalRequestSchema(AdminAPIMessageTracingSchema):
...
class V30PresRequestByFormatSchema(OpenAPISchema):
    ...
    def validate_fields(self, data, **kwargs):
        ...
        if not any(f.api in data for f in V30PresFormat.Format):
            raise ValidationError(
                "V30PresRequestByFormatSchema requires indy, dif, or both"
            )


class V30PresCreateRequestRequestSchema(AdminAPIMessageTracingSchema):
    """Request schema for creating a proof request free of any connection."""

    presentation_request = fields.Nested(V30PresRequestByFormatSchema(), required=True)
...
class V30PresSendRequestRequestSchema(V30PresCreateRequestRequestSchema):
...
class V30PresentationSendRequestToProposalSchema(AdminAPIMessageTracingSchema):
...
class V30PresSpecByFormatRequestSchema(AdminAPIMessageTracingSchema):
    ...
    def validate_fields(self, data, **kwargs):
        ...
        """
        if len(data.keys() & {f.api for f in V30PresFormat.Format}) != 1:
            raise ValidationError(
                "V30PresSpecByFormatRequestSchema must specify one presentation format"
            )


class V30CredentialsFetchQueryStringSchema(OpenAPISchema):
...
class V30PresProblemReportRequestSchema(OpenAPISchema):
    """Request schema for sending problem report."""

    description = fields.Str(required=True)


class V30PresExIdMatchInfoSchema(OpenAPISchema):
...
# TODO cleanup remove spec


def _formats_attach(by_format: Mapping, msg_type: str, spec: str) -> Mapping:
    """Break out formats and proposals/requests/presentations for v3.0 messages."""
    attach = []
    for (fmt_api, item_by_fmt) in by_format.items():
        format = V30PresFormat(format_=ATTACHMENT_FORMAT[msg_type][fmt_api])
        if fmt_api == V30PresFormat.Format.INDY.api:
            attach.append(
                AttachDecorator.data_base64(
                    mapping=item_by_fmt, ident=fmt_api, format=format
                )
            )
        elif fmt_api == V30PresFormat.Format.DIF.api:
            attach.append(
...
@docs(tags=["present-proof v3.0"], summary="Fetch all present-proof exchange records")
@querystring_schema(V30PresExRecordListQueryStringSchema)
@response_schema(V30PresExRecordListSchema(), 200, description="")
async def present_proof_list(request: web.BaseRequest):
    ...
    try:
        async with profile.session() as session:
            records = await V30PresExRecord.query(
                session=session,
...
@docs(
    tags=["present-proof v3.0"],
    summary="Fetch a single presentation exchange record",
)
@match_info_schema(V30PresExIdMatchInfoSchema())
@response_schema(V30PresExRecordSchema(), 200, description="")
async def present_proof_retrieve(request: web.BaseRequest):
    ...
    try:
        async with profile.session() as session:
            pres_ex_record = await V30PresExRecord.retrieve_by_id(session, pres_ex_id)
...
@match_info_schema(V30PresExIdMatchInfoSchema())
@querystring_schema(V30CredentialsFetchQueryStringSchema())
...
@docs(tags=["present-proof v3.0"], summary="Sends a presentation proposal")
@request_schema(V30PresProposalRequestSchema())
@response_schema(V30PresExRecordSchema(), 200, description="")
async def present_proof_send_proposal(request: web.BaseRequest):
    ...
    try:
        ...
        pres_proposal_message = V30PresProposal(
            ...
            # **_formats_attach(pres_proposal, PRES_30_PROPOSAL, "proposals"),
            attachments=_formats_attach(pres_proposal, PRES_30_PROPOSAL, "attachments"),
        )
    except (BaseModelError, StorageError) as err:
...
@request_schema(V30PresCreateRequestRequestSchema())
@response_schema(V30PresExRecordSchema(), 200, description="")
async def present_proof_create_request(request: web.BaseRequest):
    ...
    body = await request.json()  # not the body field of msg

    comment = body.get("comment")
    pres_request_spec = body.get("presentation_request")
    if pres_request_spec and V30PresFormat.Format.INDY.api in pres_request_spec:
        await _add_nonce(pres_request_spec[V30PresFormat.Format.INDY.api])

    pres_request_message = V30PresRequest(
        # comment=comment,
        # will_confirm=True,
        body=V30PresBody(comment=comment, will_confirm=True),
        # **_formats_attach(pres_request_spec, PRES_30_REQUEST, "request_presentations"),
        attachments=_formats_attach(pres_request_spec, PRES_30_REQUEST, "attachments"),
    )
...
@request_schema(V30PresSendRequestRequestSchema())
@response_schema(V30PresExRecordSchema(), 200, description="")
...
@match_info_schema(V30PresExIdMatchInfoSchema())
@request_schema(V30PresentationSendRequestToProposalSchema())
@response_schema(V30PresExRecordSchema(), 200, description="")
async def present_proof_send_bound_request(request: web.BaseRequest):
    ...
    if pres_ex_record.state != (V30PresExRecord.STATE_PROPOSAL_RECEIVED):
        raise web.HTTPBadRequest(
            reason=(
                f"Presentation exchange {pres_ex_id} "
                f"in {pres_ex_record.state} state "
                f"(must be {V30PresExRecord.STATE_PROPOSAL_RECEIVED})"
            )
        )
...
@docs(tags=["present-proof v3.0"], summary="Sends a proof presentation")
@match_info_schema(V30PresExIdMatchInfoSchema())
@request_schema(V30PresSpecByFormatRequestSchema())
@response_schema(V30PresExRecordSchema(), description="")
async def present_proof_send_presentation(request: web.BaseRequest):
    ...
    body = await request.json()
    if "dif" in body:
        fmt = V30PresFormat.Format.get("dif").api
    elif "indy" in body:
        fmt = V30PresFormat.Format.get("indy").api
    else:
        raise web.HTTPBadRequest(
            reason=(
    ...
    if pres_ex_record.state != (V30PresExRecord.STATE_REQUEST_RECEIVED):
        raise web.HTTPBadRequest(
            reason=(
                f"Presentation exchange {pres_ex_id} "
                f"in {pres_ex_record.state} state "
...
@docs(tags=["present-proof v3.0"], summary="Verify a received presentation")
@match_info_schema(V30PresExIdMatchInfoSchema())
@response_schema(V30PresExRecordSchema(), description="")
async def present_proof_verify_presentation(request: web.BaseRequest):
    ...
    if pres_ex_record.state != (V30PresExRecord.STATE_PRESENTATION_RECEIVED):
        raise web.HTTPBadRequest(
            reason=(
                f"Presentation exchange {pres_ex_id} "
                f"in {pres_ex_record.state} state "
                f"(must be {V30PresExRecord.STATE_PRESENTATION_RECEIVED})"
            )
        )
...
@match_info_schema(V30PresExIdMatchInfoSchema())
@request_schema(V30PresProblemReportRequestSchema())
@response_schema(V30PresentProofModuleResponseSchema(), 200, description="")
...
@match_info_schema(V30PresExIdMatchInfoSchema())
@response_schema(V30PresentProofModuleResponseSchema(), description="")
async def present_proof_remove(request: web.BaseRequest):
    ...
    try:
        async with context.profile.session() as session:
            try:
                pres_ex_record = await V30PresExRecord.retrieve_by_id(
                    session, pres_ex_id
                )
                ...
                storage_record = await storage.get_record(
                    record_type=V30PresExRecord.RECORD_TYPE, record_id=pres_ex_id
                )
...
async def register(app: web.Application):
    ...
    app.add_routes(
        [
            web.get(
                "/present-proof-3.0/records",
                present_proof_list,
                allow_head=False,
            ),
            web.get(
                "/present-proof-3.0/records/{pres_ex_id}",
                present_proof_retrieve,
                allow_head=False,
            ),
            web.get(
                "/present-proof-3.0/records/{pres_ex_id}/credentials",
                present_proof_credentials_list,
                allow_head=False,
            ),
            web.post(
                "/present-proof-3.0/send-proposal",
                present_proof_send_proposal,
            ),
            web.post(
                "/present-proof-3.0/create-request",
                present_proof_create_request,
            ),
            web.post(
                "/present-proof-3.0/send-request",
                present_proof_send_free_request,
            ),
            web.post(
                "/present-proof-3.0/records/{pres_ex_id}/send-request",
                present_proof_send_bound_request,
            ),
            web.post(
                "/present-proof-3.0/records/{pres_ex_id}/send-presentation",
                present_proof_send_presentation,
            ),
            web.post(
                "/present-proof-3.0/records/{pres_ex_id}/verify-presentation",
                present_proof_verify_presentation,
            ),
            web.post(
                "/present-proof-3.0/records/{pres_ex_id}/problem-report",
...
def post_process_routes(app: web.Application):
    ...
    app._state["swagger_dict"]["tags"].append(
        {
            "name": "present-proof v3.0",
            "description": "Proof presentation v3.0",
