...
from ..manager import V30PresManager, V30PresManagerError
...
class TestV30PresManager(AsyncTestCase):
    async def setUp(self):
        ...
        self.manager = V30PresManager(self.profile)
    ...
    async def test_create_exchange_for_proposal(self):
        ...
        with async_mock.patch.object(
            ...
            V30PresProposal, "serialize", autospec=True
        ):
            ...
            assert px_rec.initiator == V30PresExRecord.INITIATOR_SELF
            assert px_rec.role == V30PresExRecord.ROLE_PROVER
            assert px_rec.state == V30PresExRecord.STATE_PROPOSAL_SENT
    ...
    async def test_receive_proposal(self):
        ...
        with async_mock.patch.object(V30PresExRecord, "save", autospec=True) as save_ex:
            ...
            assert px_rec.state == V30PresExRecord.STATE_PROPOSAL_RECEIVED
    ...
    async def test_create_exchange_for_request(self):
        ...
        with async_mock.patch.object(V30PresExRecord, "save", autospec=True) as save_ex:
            ...
            assert px_rec.initiator == V30PresExRecord.INITIATOR_SELF
            assert px_rec.role == V30PresExRecord.ROLE_VERIFIER
            assert px_rec.state == V30PresExRecord.STATE_REQUEST_SENT
    ...
    async def test_receive_pres_request(self):
        ...
        with async_mock.patch.object(V30PresExRecord, "save", autospec=True) as save_ex:
            ...
            assert px_rec_out.state == V30PresExRecord.STATE_REQUEST_RECEIVED
    ...
    async def test_no_matching_creds_indy_handler(self):
        ...
        with async_mock.patch.object(
            ...
            with self.assertRaises(
                test_indy_handler.V30PresFormatHandlerError
            ) as context:
                (px_rec_out, pres_msg) = await self.manager.create_pres(
                    px_rec_in, request_data
                )
    ...
    async def test_receive_problem_report(self):
        ...
        stored_exchange = V30PresExRecord(
        ...
        assert stored_exchange.state == V30PresExRecord.STATE_ABANDONED
