...
class V30PresHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30PresHandler called with context %s", context)
        assert isinstance(context.message, V30Pres)
        self._logger.info(
        ...
        r_time = trace_event(
            context.settings,
            context.message,
            outcome="V30PresHandler.handle.END",
            perf_counter=r_time,
        )
        ...
        if pres_ex_record and pres_ex_record.auto_verify:
            ...
            trace_event(
                context.settings,
                pres_ex_record,
                outcome="V30PresHandler.handle.VERIFY",
                perf_counter=r_time,
            )
