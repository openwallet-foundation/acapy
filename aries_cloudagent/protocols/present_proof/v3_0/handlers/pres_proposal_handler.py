...
class V30PresProposalHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30PresProposalHandler called with context %s", context)
        assert isinstance(context.message, V30PresProposal)
        self._logger.info(
        ...
        r_time = trace_event(
            context.settings,
            context.message,
            outcome="V30PresProposalHandler.handle.END",
            perf_counter=r_time,
        )
        ...
        if context.settings.get("debug.auto_respond_presentation_proposal"):
            ...
            trace_event(
                context.settings,
                pres_request_message,
                outcome="V30PresProposalHandler.handle.PRESENT",
                perf_counter=r_time,
            )
