...
from ...handler import V30PresFormatHandlerError
...
class TestDIFFormatHandler(AsyncTestCase):
    ...
    def test_validate_fields(self):
        self.handler.validate_fields(PRES_30, DIF_PRES)
        self.handler.validate_fields(PRES_30_PROPOSAL, DIF_PRES_PROPOSAL)
        self.handler.validate_fields(PRES_30_REQUEST, DIF_PRES_REQUEST_A)
        self.handler.validate_fields(PRES_30_REQUEST, DIF_PRES_REQUEST_B)

        with self.assertRaises(ValidationError):
            incorrect_pres = DIF_PRES.copy()
            incorrect_pres.pop("@context")
            self.handler.validate_fields(PRES_30, incorrect_pres)
    ...
    async def test_create_bound_request_a(self):
        ...
        dif_pres_proposal = V30PresProposal(
            body=V30PresBody(),
            attachments=[
        ...
        assert isinstance(output[0], V30PresFormat) and isinstance(
    ...
    async def test_create_pres(self):
        dif_pres_request = V30PresRequest(
            body=V30PresBody(),
            attachments=[
                AttachDecorator.data_json(
                    DIF_PRES_REQUEST_B,
    ...
    async def test_create_pres_storage_not_found(self):
        ...
        with self.assertRaises(V30PresFormatHandlerError):
