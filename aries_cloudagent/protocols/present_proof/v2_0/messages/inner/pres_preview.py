...
class V20PresPredSpecSchema(BaseModelSchema):
    """Predicate specifiation schema."""

    class Meta:
        """Predicate specifiation schema metadata."""

        model_class = V20PresPredSpec
...
class V20PresAttrSpec(BaseModel):
    ...
    def list_plain(plain: dict, cred_def_id: str, referent: str = None):
        """
        Return a list of `V20PresAttrSpec` on input cred def id.
        ...
        Returns:
            List of V20PresAttrSpec on input cred def id with no MIME types
    ...
    @property
    def posture(self) -> "V20PresAttrSpec.Posture":
        """Attribute posture: self-attested, revealed claim, or unrevealed claim."""

        if self.cred_def_id:
            if self.value:
                return V20PresAttrSpec.Posture.REVEALED_CLAIM
            return V20PresAttrSpec.Posture.UNREVEALED_CLAIM
        if self.value:
            return V20PresAttrSpec.Posture.SELF_ATTESTED
    ...
    def satisfies(self, pred_spec: V20PresPredSpec):
...
class V20PresAttrSpecSchema(BaseModelSchema):
    """Attribute specifiation schema."""

    class Meta:
        """Attribute specifiation schema metadata."""

        model_class = V20PresAttrSpec
...
class V20PresPreview(BaseModel):
    """Class representing presentation preview."""

    class Meta:
        """Presentation preview metadata."""

        schema_class = "V20PresPreviewSchema"
        message_type = PRES_20_PREVIEW

    def __init__(
        self,
        *,
        _type: str = None,
        attributes: Sequence[V20PresAttrSpec] = None,
        predicates: Sequence[V20PresPredSpec] = None,
        **kwargs,
    ):
        """
    ...
    def _type(self):
        ...
        return DIDCommPrefix.qualify_current(PresPreview.Meta.message_type)
    ...
    async def indy_proof_request(
        ...
        for attr_spec in self.attributes:
            if attr_spec.posture == V20PresAttrSpec.Posture.SELF_ATTESTED:
...
class V20PresPreviewSchema(BaseModelSchema):
    """Presentation preview schema."""

    class Meta:
        """Pres preview schema metadata."""

        model_class = V20PresPreview

    _type = fields.Str(
        description="Message type identifier",
        required=False,
        example=DIDCommPrefix.qualify_current(PRES_PREVIEW),
        data_key="@type",
    )
    attributes = fields.Nested(V20PresAttrSpecSchema, required=True, many=True)
    predicates = fields.Nested(V20PresPredSpecSchema, required=True, many=True)
