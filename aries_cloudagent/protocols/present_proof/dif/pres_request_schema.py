...
class DIFProofRequestSchema(BaseModelSchema):
    ...
    presentation_definition = fields.Nested(PresentationDefinitionSchema(),
        required=True)
...
class DIFPresSpecSchema(OpenAPISchema):
    ...
    record_ids = fields.Dict(required=False, metadata={'description':
        'Mapping of input_descriptor id to list of stored W3C credential record_id'
        , 'example': {'<input descriptor id_1>': ['<record id_1>',
        '<record id_2>'], '<input descriptor id_2>': ['<record id>']}})
    presentation_definition = fields.Nested(PresentationDefinitionSchema(),
        required=False)
    reveal_doc = fields.Dict(required=False, metadata={'description':
        ...
        'https://w3id.org/security/bbs/v1'], 'type': ['VerifiableCredential',
        'LabReport'], '@explicit': True, '@requireAll': True, 'issuanceDate': {
        }, 'issuer': {}, 'credentialSubject': {'Observation': [{
        'effectiveDateTime': {}, '@explicit': True, '@requireAll': True}],
        '@explicit': True, '@requireAll': True}}})
