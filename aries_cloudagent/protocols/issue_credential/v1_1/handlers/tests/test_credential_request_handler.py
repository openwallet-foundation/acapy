...
from ...messages.inner.credential_preview import V11CredAttrSpec, V11CredentialPreview
...
class TestCredentialRequestHandler(AsyncTestCase):
    ...
    async def test_called_auto_issue(self):
        ...
        cred_ex_rec = V11CredentialExchange(
            credential_proposal_dict={
                "credential_proposal": V11CredentialPreview(
                    attributes=(V11CredAttrSpec.list_plain(ATTR_DICT))
                ).serialize(),
                "cred_def_id": CD_ID
            }
        )
