...
class V11CredAttrSpec(BaseModel):
    ...
    def list_plain(plain: dict):
        """
        Return a list of `V11CredAttrSpec` without MIME types from names/values.

        Args:
            plain: dict mapping names to values

        Returns:
            List of V11CredAttrSpecs with no MIME types

        """
        return [V11CredAttrSpec(name=k, value=plain[k]) for k in plain]
...
class V11CredAttrSpecSchema(BaseModelSchema):
    """Attribute preview schema."""

    class Meta:
        """Attribute preview schema metadata."""

        model_class = V11CredAttrSpec
...
class V11CredentialPreview(BaseModel):
    ...
    def __init__(
        ...
        *,
        _type: str = None,
        attributes: Sequence[V11CredAttrSpec] = None,
        **kwargs
    ):
        """
    ...
    def _type(self):
        ...
        return V11CredentialPreview.Meta.message_type
...
class V11CredentialPreviewSchema(BaseModelSchema):
    """Credential preview schema."""

    class Meta:
        """Credential preview schema metadata."""

        model_class = V11CredentialPreview
    ...
    attributes = fields.Nested(
        V11CredAttrSpecSchema, many=True, required=True, data_key="attributes"
    )
