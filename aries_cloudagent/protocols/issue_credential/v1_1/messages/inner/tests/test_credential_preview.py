...
CRED_PREVIEW = V11CredentialPreview(
    attributes=(
        V11CredAttrSpec.list_plain({"test": "123", "hello": "world"})
        + [V11CredAttrSpec(name="icon", value="cG90YXRv", mime_type="image/PNG")]
    )
)


class TestV11CredAttrSpec(TestCase):
    """Attribute preview tests"""

    def test_eq(self):
        attr_previews_none_plain = [
            V11CredAttrSpec(name="item", value="value"),
            V11CredAttrSpec(name="item", value="value", mime_type=None),
        ]
        attr_previews_different = [
            V11CredAttrSpec(name="item", value="dmFsdWU=", mime_type="image/png"),
            V11CredAttrSpec(name="item", value="distinct value"),
            V11CredAttrSpec(name="distinct_name", value="distinct value", mime_type=None),
        ]
...
class TestV11CredentialPreview(TestCase):
    ...
    def test_deserialize(self):
        ...
        cred_preview = V11CredentialPreview.deserialize(obj)
        assert type(cred_preview) == V11CredentialPreview
...
class TestV11CredentialPreviewSchema(TestCase):
    """Test credential cred preview schema."""

    def test_make_model(self):
        """Test making model."""
        data = CRED_PREVIEW.serialize()
        model_instance = V11CredentialPreview.deserialize(data)
        assert isinstance(model_instance, V11CredentialPreview)
