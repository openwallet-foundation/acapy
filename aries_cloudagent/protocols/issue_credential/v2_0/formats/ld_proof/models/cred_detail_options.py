...
class CredentialStatusOptionsSchema(Schema):
    ...
    type = fields.Str(required=True, metadata={'description':
...
class LDProofVCDetailOptionsSchema(BaseModelSchema):
    ...
    proof_type = fields.Str(data_key='proofType', required=True, metadata={
        'description':
        'The proof type used for the proof. Should match suites registered in the Linked Data Cryptographic Suite Registry'
        , 'example': 'Ed25519Signature2018'})

    proof_purpose = fields.Str(data_key='proofPurpose', required=False,
    ...
    created = fields.Str(required=False, validate=
    ...
    domain = fields.Str(required=False, metadata={'description':
        'The intended domain of validity for the proof', 'example': 'example.com'})

    challenge = fields.Str(required=False, metadata={'description':
        'A challenge to include in the proof. SHOULD be provided by the requesting party of the credential (=holder)'
        , 'example': UUIDFour.EXAMPLE})

    credential_status = fields.Nested(CredentialStatusOptionsSchema(), data_key
        ='credentialStatus', required=False, metadata={'description':
