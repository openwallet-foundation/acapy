"""V3.0 issue-credential indy credential format handler."""
...
from ..handler import CredFormatAttachment, V30CredFormatError, V30CredFormatHandler

LOGGER = logging.getLogger(__name__)


class IndyCredFormatHandler(V30CredFormatHandler):
    """Indy credential format handler."""

    format = V30CredFormat.Format.INDY
    ...
    def validate_fields(cls, message_type: str, attachment_data: Mapping):
        ...
        """
        mapping = {
            CRED_30_PROPOSAL: CredDefQueryStringSchema,
            CRED_30_OFFER: IndyCredAbstractSchema,
            CRED_30_REQUEST: IndyCredRequestSchema,
            CRED_30_ISSUE: IndyCredentialSchema,
        }
    ...
    async def get_detail_record(self, cred_ex_id: str) -> V30CredExRecordIndy:
    ...
    def get_format_data(self, message_type: str, data: dict) -> CredFormatAttachment:
        ...
        return (
            ...
            AttachDecorator.data_base64(
                data, ident=IndyCredFormatHandler.format.api, format=format
            ),
        )
    ...
    async def create_proposal(
        self, cred_ex_record: V30CredExRecord, proposal_data: Mapping[str, str]
    ) -> Tuple[V30CredFormat, AttachDecorator]:
        """Create indy credential proposal."""
        if proposal_data is None:
            proposal_data = {}

        return self.get_format_data(CRED_30_PROPOSAL, proposal_data)
    ...
    async def create_offer(
        ...
        preview_attrs = {
            attr
            for attr in cred_proposal_message._body.credential_preview._body.attr_dict()
        }
        ...
        return self.get_format_data(CRED_30_OFFER, cred_offer)
    ...
    async def create_request(
        ...
        if "nonce" not in cred_offer:
            raise V30CredFormatError("Missing nonce in credential offer")
        ...
        detail_record = V30CredExRecordIndy(
            cred_ex_id=cred_ex_record.cred_ex_id,
            cred_request_metadata=cred_req_result["metadata"],
        )

        async with self.profile.session() as session:
            await detail_record.save(session, reason="create v3.0 credential request")

        return self.get_format_data(CRED_30_REQUEST, cred_req_result["request"])
    ...
    async def issue_credential(
        ...
        for attempt in range(max(retries, 1)):
            ...
            result = self.get_format_data(CRED_30_ISSUE, json.loads(cred_json))
            break
        ...
        async with self._profile.transaction() as txn:
            ...
            await detail_record.save(txn, reason="v3.0 issue credential")
    ...
    async def store_credential(
        ...
        mime_types = None
        if cred_offer_message and cred_offer_message._body.credential_preview:
            mime_types = (
                cred_offer_message._body.credential_preview._body.mime_types() or None
            )
        ...
        try:
            ...
            async with self.profile.session() as session:
                ...
                await detail_record.save(
                    session, reason="store credential v3.0", event=True
                )
