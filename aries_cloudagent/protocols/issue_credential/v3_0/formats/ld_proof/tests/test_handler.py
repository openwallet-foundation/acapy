...
class TestV30LDProofCredFormatHandler(AsyncTestCase):
    ...
    async def test_validate_fields(self):
        # Test correct data
        self.handler.validate_fields(CRED_30_PROPOSAL, LD_PROOF_VC_DETAIL)
        self.handler.validate_fields(CRED_30_OFFER, LD_PROOF_VC_DETAIL)
        self.handler.validate_fields(CRED_30_REQUEST, LD_PROOF_VC_DETAIL)
        self.handler.validate_fields(CRED_30_ISSUE, LD_PROOF_VC)
        ...
        with self.assertRaises(ValidationError):
            self.handler.validate_fields(CRED_30_PROPOSAL, incorrect_detail)

        # test incorrect offer
        with self.assertRaises(ValidationError):
            self.handler.validate_fields(CRED_30_OFFER, incorrect_detail)

        # test incorrect request
        with self.assertRaises(ValidationError):
            self.handler.validate_fields(CRED_30_REQUEST, incorrect_detail)
        ...
        with self.assertRaises(ValidationError):
            ...
            self.handler.validate_fields(CRED_30_ISSUE, incorrect_cred)
