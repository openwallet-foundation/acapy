...
from ...message_types import ATTACHMENT_FORMAT, CRED_30_ISSUE

from .. import cred_issue as test_module
from ..cred_format import V30CredFormat
from ..cred_issue import V30CredIssue


class TestV30CredIssue(AsyncTestCase):
    ...
    CRED_ISSUE = V30CredIssue(
        _body=V30CredBody(replacement_id="0", comment="Test"),
        attachments=[
    ...
    async def test_init_type(self):
        ...
        assert (
            TestV30CredIssue.CRED_ISSUE.attachments[0].content
            == TestV30CredIssue.INDY_CRED
        )
        assert TestV30CredIssue.CRED_ISSUE.attachments[0].ident  # auto-generates UUID4
        assert TestV30CredIssue.CRED_ISSUE._type == DIDCommPrefix.qualify_current(
            CRED_30_ISSUE
        )
    ...
    async def test_attachment_no_target_format(self):
        ...
        x_cred = V30CredIssue(
            ...
            attachments=[
                AttachDecorator.data_base64(
                    ident="not_indy",
                    mapping=TestV30CredIssue.CRED_ISSUE.serialize(),
    ...
    async def test_deserialize(self):
        ...
        obj = TestV30CredIssue.CRED_ISSUE.serialize()

        cred_issue = V30CredIssue.deserialize(obj)
        assert type(cred_issue) == V30CredIssue

        obj["attachments"][0]["data"]["base64"] = "eyJub3QiOiAiaW5keSJ9"
        with self.assertRaises(BaseModelError):
            V30CredIssue.deserialize(obj)
    ...
    async def test_serialize(self):
        """Test serialization."""
        obj = TestV30CredIssue.CRED_ISSUE

        with async_mock.patch.object(
            test_module.V30CredIssueSchema, "dump", async_mock.MagicMock()
        ) as mock_dump:
    ...
    async def test_make_model(self):
        ...
        data = TestV30CredIssue.CRED_ISSUE.serialize()
        model_instance = V30CredIssue.deserialize(data)
        assert isinstance(model_instance, V30CredIssue)
