...
class V30CredProposalHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30CredProposalHandler called with context %s", context)
        assert isinstance(context.message, V30CredProposal)
        self._logger.info(
            "Received v3.0 credential proposal message: %s",
        ...
        if cred_ex_record and cred_ex_record.auto_offer:
            ...
            trace_event(
                context.settings,
                cred_offer_message,
                outcome="V30CredProposalHandler.handle.OFFER",
                perf_counter=r_time,
            )
