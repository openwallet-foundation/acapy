...
class V30CredRequestHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30CredRequestHandler called with context %s", context)
        assert isinstance(context.message, V30CredRequest)
        self._logger.info(
            "Received v3.0 credential request message: %s",
        ...
        r_time = trace_event(
            context.settings,
            context.message,
            outcome="V30CredRequestHandler.handle.END",
            perf_counter=r_time,
        )
        ...
        if cred_ex_record and cred_ex_record.auto_issue:
            ...
            try:
                try:
                    comment = context.message._body.comment
            ...
            trace_event(
                context.settings,
                cred_issue_message,
                outcome="V30CredRequestHandler.issue.END",
                perf_counter=r_time,
            )
