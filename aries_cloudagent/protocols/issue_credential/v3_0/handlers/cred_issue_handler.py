...
class V30CredIssueHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30CredIssueHandler called with context %s", context)
        assert isinstance(context.message, V30CredIssue)
        self._logger.info(
            "Received v3.0 credential issue message: %s",
        ...
        r_time = trace_event(
            context.settings,
            context.message,
            outcome="V30CredIssueHandler.handle.END",
            perf_counter=r_time,
        )
        ...
        if context.settings.get("debug.auto_store_credential"):
            ...
            trace_event(
                context.settings,
                cred_ack_message,
                outcome="V30CredIssueHandler.handle.STORE",
                perf_counter=r_time,
            )
