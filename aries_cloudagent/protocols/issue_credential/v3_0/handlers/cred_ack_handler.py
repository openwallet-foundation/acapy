...
from ..manager import V30CredManager
from ..messages.cred_ack import V30CredAck


class V30CredAckHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30CredAckHandler called with context %s", context)
        assert isinstance(context.message, V30CredAck)
        self._logger.info(
        ...
        trace_event(
            context.settings,
            context.message,
            outcome="V30CredAckHandler.handle.END",
            perf_counter=r_time,
        )
