...
class V30CredOfferHandler(BaseHandler):
    ...
    async def handle(self, context: RequestContext, responder: BaseResponder):
        ...
        self._logger.debug("V30CredOfferHandler called with context %s", context)
        assert isinstance(context.message, V30CredOffer)
        self._logger.info(
        ...
        r_time = trace_event(
            context.settings,
            context.message,
            outcome="V30CredOfferHandler.handle.END",
            perf_counter=r_time,
        )
        ...
        if context.settings.get("debug.auto_respond_credential_offer"):
            ...
            trace_event(
                ...
                outcome="V30CredOfferHandler.handle.REQUEST",
                perf_counter=r_time,
            )
