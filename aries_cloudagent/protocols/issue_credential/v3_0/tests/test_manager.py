...
from ..messages.cred_problem_report import V30CredProblemReport
...
CRED_ISSUE = V30CredIssue(
    ...
    attachments=[
        AttachDecorator.data_base64(
            ...
            ident="indy",
            format=V30CredFormat(
                format_=ATTACHMENT_FORMAT[CRED_30_ISSUE][V30CredFormat.Format.INDY.api],
            ),
        ),
    ],
)
...
class TestV30CredManager(AsyncTestCase):
    async def setUp(self):
        ...
        self.manager = V30CredManager(self.profile)
    ...
    async def test_prepare_send(self):
        ...
        with async_mock.patch.object(
            ...
            assert arg_cred_ex_rec.role == V30CredExRecord.ROLE_ISSUER
    ...
    async def test_create_proposal(self):
        ...
        with async_mock.patch.object(
            ...
            mock_handler.return_value.create_proposal = async_mock.CoroutineMock(
                return_value=(
                    V30CredFormat(
                        attach_id=V30CredFormat.Format.INDY.api,
                        format_=ATTACHMENT_FORMAT[CRED_30_PROPOSAL][
                            V30CredFormat.Format.INDY.api
                        ],
                    ),
                    AttachDecorator.data_base64(
                        {},
                        ident=V30CredFormat.Format.INDY.api,
                        format=V30CredFormat(
                            format_=ATTACHMENT_FORMAT[CRED_30_PROPOSAL][
                                V30CredFormat.Format.INDY.api
                            ],
            ...
            cx_rec = await self.manager.create_proposal(
                ...
                fmt2filter={V30CredFormat.Format.INDY: None},
        ...
        assert cx_rec.role == V30CredExRecord.ROLE_HOLDER
        assert cx_rec.state == V30CredExRecord.STATE_PROPOSAL_SENT
    ...
    async def test_create_proposal_no_preview(self):
        ...
        with async_mock.patch.object(
            ...
            mock_handler.return_value.create_proposal = async_mock.CoroutineMock(
                return_value=(
                    V30CredFormat(
                        attach_id=V30CredFormat.Format.LD_PROOF.api,
                        format_=ATTACHMENT_FORMAT[CRED_30_PROPOSAL][
                    ...
                    AttachDecorator.data_base64(
                        LD_PROOF_VC_DETAIL,
                        ident=V30CredFormat.Format.LD_PROOF.api,
                        format=V30CredFormat(
                            format_=ATTACHMENT_FORMAT[CRED_30_PROPOSAL][
                                V30CredFormat.Format.LD_PROOF.api
                            ],
            ...
            cx_rec = await self.manager.create_proposal(
                ...
                comment=comment,
                cred_preview=None,
                fmt2filter={V30CredFormat.Format.LD_PROOF: LD_PROOF_VC_DETAIL},
            )
            mock_save.assert_called_once()
            mock_handler.return_value.create_proposal.assert_called_once_with(
        ...
        cred_proposal = cx_rec.cred_proposal
        assert (
            cred_proposal.attachment(V30CredFormat.Format.LD_PROOF)
            == LD_PROOF_VC_DETAIL
        )
        assert cx_rec.connection_id == connection_id
        assert cx_rec.thread_id == cred_proposal._thread_id
    ...
    async def test_receive_proposal(self):
        ...
        with async_mock.patch.object(
            ...
            assert ret_cred_proposal.attachment(V30CredFormat.Format.INDY) == {
                "cred_def_id": CRED_DEF_ID
            }
            assert (
                ret_cred_proposal._body.credential_preview._body.attributes
                == cred_preview._body.attributes
            )
            assert cx_rec.connection_id == connection_id
            assert cx_rec.role == V30CredExRecord.ROLE_ISSUER
            assert cx_rec.state == V30CredExRecord.STATE_PROPOSAL_RECEIVED
            assert cx_rec.thread_id == cred_proposal._thread_id
    ...
    async def test_create_free_offer(self):
        ...
        with async_mock.patch.object(
            ...
            assert cx_rec.role == V30CredExRecord.ROLE_ISSUER
            assert cx_rec.state == V30CredExRecord.STATE_OFFER_SENT
            assert cx_rec.cred_offer.attachment(V30CredFormat.Format.INDY) == INDY_OFFER
    ...
    async def test_create_bound_offer(self):
        ...
        with async_mock.patch.object(
            ...
            assert (
                ret_offer._body.credential_preview._body.attributes
                == cred_preview._body.attributes
            )
    ...
    async def test_receive_offer_proposed(self):
        ...
        with async_mock.patch.object(
            ...
            assert cx_rec.role == V30CredExRecord.ROLE_HOLDER
            assert cx_rec.state == V30CredExRecord.STATE_OFFER_RECEIVED
            assert cx_rec.cred_offer.attachment(V30CredFormat.Format.INDY) == INDY_OFFER
            assert (
                cx_rec.cred_proposal._body.credential_preview._body.attributes
                == cred_preview._body.attributes
            )
    ...
    async def test_create_bound_request(self):
        ...
        stored_cx_rec = V30CredExRecord(
            ...
            role=V30CredExRecord.ROLE_HOLDER,
            state=V30CredExRecord.STATE_OFFER_RECEIVED,
            thread_id=thread_id,
        )
    ...
    async def test_create_request_existing_cred_req(self):
        stored_cx_rec = V30CredExRecord(cred_request=CRED_REQ)
    ...
    async def test_issue_credential(self):
        ...
        stored_cx_rec = V30CredExRecord(
            ...
            initiator=V30CredExRecord.INITIATOR_SELF,
            role=V30CredExRecord.ROLE_ISSUER,
            state=V30CredExRecord.STATE_REQUEST_RECEIVED,
            thread_id=thread_id,
        )
    ...
    async def test_receive_cred_x_no_formats(self):
        ...
        cred_issue = V30CredIssue(
            attachments=[
                AttachDecorator.data_base64(
                    LD_PROOF_VC, ident="0", format=V30CredFormat(format_="random")
                )
            ],
        )
    ...
    async def test_store_credential(self):
        ...
        stored_cx_rec = V30CredExRecord(
            ...
            initiator=V30CredExRecord.INITIATOR_EXTERNAL,
            role=V30CredExRecord.ROLE_ISSUER,
            state=V30CredExRecord.STATE_CREDENTIAL_RECEIVED,
            auto_remove=True,
            thread_id=thread_id,
        )
        ...
        with async_mock.patch.object(
            ...
            test_module.V30CredManager, "delete_cred_ex_record", autospec=True
    ...
    async def test_send_cred_ack(self):
        ...
        stored_exchange = V30CredExRecord(
            cred_ex_id="dummy-cxid",
            connection_id=connection_id,
            initiator=V30CredExRecord.INITIATOR_SELF,
            thread_id="thid",
        ...
        with async_mock.patch.object(
            ...
            V30CredExRecord, "delete_record", autospec=True
            ...
            assert ack._thread
            assert cx_rec.state == V30CredExRecord.STATE_DONE
    ...
    async def test_receive_cred_ack(self):
        ...
        with async_mock.patch.object(
            ...
            assert ret_cx_rec.state == V30CredExRecord.STATE_DONE
    ...
    async def test_delete_cred_ex_record(self):
        ...
        with async_mock.patch.object(
            ...
            V30CredExRecord, "retrieve_by_id", async_mock.CoroutineMock()
        ) as mock_retrieve, async_mock.patch.object(
            test_module, "V30CredFormat", async_mock.MagicMock()
        ) as mock_cred_format:
            mock_retrieve.return_value = stored_cx_rec
    ...
    async def test_receive_problem_report(self):
        ...
        with async_mock.patch.object(
            ...
            assert ret_exchange.state == V30CredExRecord.STATE_ABANDONED
    ...
    async def test_retrieve_records(self):
        ...
        for i in range(2):  # second pass gets from cache
            for index in range(2):
                ret_ex = await V30CredExRecord.retrieve_by_conn_and_thread(
