...
from .manager import V30CredManager, V30CredManagerError
from .message_types import ATTACHMENT_FORMAT, CRED_30_PROPOSAL, SPEC_URI
...
from .messages.inner.cred_preview import V30CredPreview, V30CredPreviewSchema
from .models.cred_ex_record import V30CredExRecord, V30CredExRecordSchema
from .models.detail.ld_proof import V30CredExRecordLDProofSchema
from .models.detail.indy import V30CredExRecordIndySchema
from .formats.handler import V30CredFormatError
from .messages.cred_body import V30CredBody
from .formats.ld_proof.models.cred_detail import LDProofVCDetailSchema

LOGGER = logging.getLogger(__name__)


class V30IssueCredentialModuleResponseSchema(OpenAPISchema):
    """Response schema for v3.0 Issue Credential Module."""


class V30CredExRecordListQueryStringSchema(OpenAPISchema):
...
class V30CredFilterSchema(OpenAPISchema):
    """Credential filtration criteria."""

    indy = fields.Nested(
        V30CredFilterIndySchema,
        required=False,
    ...
    def validate_fields(self, data, **kwargs):
        ...
        """
        if not any(f.api in data for f in V30CredFormat.Format):
            raise ValidationError(
                "V30CredFilterSchema requires indy, ld_proof, or both"
            )


class V30IssueCredSchemaCore(AdminAPIMessageTracingSchema):
    ...
    credential_preview = fields.Nested(V30CredPreviewSchema, required=False)
...
class V30CredFilterLDProofSchema(OpenAPISchema):
    """Credential filtration criteria."""
...
class V30CredRequestFreeSchema(AdminAPIMessageTracingSchema):
    """Filter, auto-remove, comment, trace."""
...
class V30CredExFreeSchema(V30IssueCredSchemaCore):
...
class V30CredBoundOfferRequestSchema(OpenAPISchema):
    ...
    def validate_fields(self, data, **kwargs):
        ...
        if (
            ...
            raise ValidationError(
                f"V30CredBoundOfferRequestSchema\n{data}\nrequires "
                "both indy/ld_proof filter and counter_preview or neither"
            )


class V30CredOfferRequestSchema(V30IssueCredSchemaCore):
...
class V30CredOfferConnFreeRequestSchema(V30IssueCredSchemaCore):
...
class V30CredRequestRequestSchema(OpenAPISchema):
...
class V30CredIssueRequestSchema(OpenAPISchema):
...
class V30CredIssueProblemReportRequestSchema(OpenAPISchema):
    """Request schema for sending problem report."""

    description = fields.Str(required=True)


class V30CredIdMatchInfoSchema(OpenAPISchema):
...
class V30CredExIdMatchInfoSchema(OpenAPISchema):
...
def _formats_filters(filt_spec: Mapping) -> Mapping:
    """Break out formats and filters for v3.0 cred proposal messages."""
    attach = []
    if filt_spec:
        for fmt_api in filt_spec:
            format = V30CredFormat(format_=ATTACHMENT_FORMAT[CRED_30_PROPOSAL][fmt_api])
...
async def _get_attached_credentials(
    profile: Profile, cred_ex_record: V30CredExRecord
) -> Mapping:
...
def _format_result_with_details(
    cred_ex_record: V30CredExRecord, details: Mapping
) -> Mapping:
...
async def credential_exchange_list(request: web.BaseRequest):
    ...
    try:
        async with profile.session() as session:
            cred_ex_records = await V30CredExRecord.query(
                session=session,
...
async def credential_exchange_create(request: web.BaseRequest):
    ...
    try:
        ...
        cred_preview = (
            V30CredPreview.deserialize(preview_spec) if preview_spec else None
        )
        ...
        cred_manager = V30CredManager(context.profile)
...
async def credential_exchange_send_proposal(request: web.BaseRequest):
    ...
    try:
        ...
        cred_ex_record = await cred_manager.create_proposal(
            ...
            trace=trace_msg,
            fmt2filter={
                V30CredFormat.Format.get(fmt_api): filt_by_fmt
                for (fmt_api, filt_by_fmt) in filt_spec.items()
            },
        )
...
async def _create_free_offer(
    ...
    (cred_ex_record, cred_offer_message) = await cred_manager.create_offer(
        cred_ex_record,
        counter_proposal=cred_proposal,
        comment=comment,
    )
...
async def credential_exchange_send_bound_offer(request: web.BaseRequest):
    ...
    try:
        async with profile.session() as session:
            try:
                cred_ex_record = await V30CredExRecord.retrieve_by_id(
                    session,
                    cred_ex_id,
                )
            except StorageNotFoundError as err:
            ...
            if cred_ex_record.state != (
                V30CredExRecord.STATE_PROPOSAL_RECEIVED
                ...
                raise V30CredManagerError(
                    ...
                    f"(must be {V30CredExRecord.STATE_PROPOSAL_RECEIVED})"
                )
...
async def register(app: web.Application):
    ...
    app.add_routes(
        [
            web.get(
                "/issue-credential-3.0/records",
                credential_exchange_list,
                allow_head=False,
            ),
            web.post(
                "/issue-credential-3.0/create-offer",
                credential_exchange_create_free_offer,
            ),
            web.get(
                "/issue-credential-3.0/records/{cred_ex_id}",
                credential_exchange_retrieve,
                allow_head=False,
            ),
            web.post("/issue-credential-3.0/create", credential_exchange_create),
            web.post("/issue-credential-3.0/send", credential_exchange_send),
            web.post(
                "/issue-credential-3.0/send-proposal", credential_exchange_send_proposal
            ),
            web.post(
                "/issue-credential-3.0/send-offer", credential_exchange_send_free_offer
            ),
            web.post(
                "/issue-credential-3.0/send-request",
                credential_exchange_send_free_request,
            ),
            web.post(
                "/issue-credential-3.0/records/{cred_ex_id}/send-offer",
                credential_exchange_send_bound_offer,
            ),
            web.post(
                "/issue-credential-3.0/records/{cred_ex_id}/send-request",
                credential_exchange_send_bound_request,
            ),
            web.post(
                "/issue-credential-3.0/records/{cred_ex_id}/issue",
                credential_exchange_issue,
            ),
            web.post(
                "/issue-credential-3.0/records/{cred_ex_id}/store",
                credential_exchange_store,
            ),
            web.post(
                "/issue-credential-3.0/records/{cred_ex_id}/problem-report",
...
def post_process_routes(app: web.Application):
    ...
    app._state["swagger_dict"]["tags"].append(
        {
            "name": "issue-credential v3.0",
            "description": "Credential issue v3.0",
