...
class V10CredentialExchange(BaseModel):
    ...
    async def retrieve_by_tag_filter(
        ...
        vals = json.loads(result.value)
        vals.update(result.tags)
        return V10CredentialExchange(credential_exchange_id=result.id, **vals)

    @classmethod
    async def query(
        cls, context: InjectionContext, tag_filter: dict = None
    ) -> Sequence["V10CredentialExchange"]:
        ...
        for record in found:
            ...
            result.append(
                V10CredentialExchange(credential_exchange_id=record.id, **vals)
            )
        return result
...
class V10CredentialExchangeSchema(BaseModelSchema):
