...
def mock_request(mock_sign_credential, mock_verify_credential):
    context = AdminRequestContext.test_context(
        {
            credential.sign_credential: mock_sign_credential,
            credential.verify_credential: mock_verify_credential,
        }
    )
...
@pytest.mark.asyncio
async def test_sign(mock_request, mock_response, mock_sign_credential):
...
async def test_sign_key_not_found_error(
    mock_request, mock_response, mock_sign_credential
):
    mock_sign_credential = async_mock.CoroutineMock(side_effect=StorageError())
...
async def test_sign_bad_req_error(
    mock_request, mock_response, mock_sign_credential, error
):
    mock_sign_credential = async_mock.CoroutineMock(side_effect=error())
