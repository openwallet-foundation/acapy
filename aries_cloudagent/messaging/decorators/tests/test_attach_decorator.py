...
class TestAttachDecorator(TestCase):
    ...
    def test_jws2(self):
        ...
        jws2 = AttachDecoratorDataJWS(
            signatures=[one_jws, one_jws]
        )
...
class TestAttachDecoratorSignature:
    ...
    async def test_indy_sign(self, wallet, seed):
        ...
        assert deco_indy.data.header_map()["kid"] == did_key(
            did_info[0].verkey
        )
        assert deco_indy.data.header_map()["jwk"]["kid"] == did_key(
            did_info[0].verkey
        )
        ...
        tampered = bytearray(
            b64_to_bytes(deco_indy.data.jws.signature, urlsafe=True)
        )
        ...
        await deco_indy.data.sign(
            [did_info[i].verkey for i in range(len(did_info))],
            wallet
        )
        assert deco_indy.data.jws is not None
        ...
        for i in range(len(did_info)):
            ...
            assert deco_indy.data.header_map(i)["kid"] == did_key(
                did_info[i].verkey
            )
