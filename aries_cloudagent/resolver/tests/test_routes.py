...
def mock_resolver(resolution_result):
    ...
    did_resolver.resolve_with_metadata = mock.AsyncMock(return_value=resolution_result)
    yield did_resolver
...
async def test_resolver_not_found_error(mock_resolver, side_effect, error):
    mock_resolver.resolve_with_metadata = mock.AsyncMock(side_effect=side_effect())
