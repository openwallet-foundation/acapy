...
def Test_Hex_Example():
    ...
    aliceKey = deriveSender1PU(aliceEphemeralSecretKey, aliceSecretKey, bobPublicKey, alg, apu, apv, keydatalen)
    print("Alice 1PU key: ", aliceKey.hex())
    bobKey = deriveReceiver1PU(aliceEphemeralPublicKey, alicePublicKey, bobSecretKey, alg, apu, apv, keydatalen)
...
def Test_Appendix_Example():
    ...
    # Pad the base64 values before decoding
    ...
    missing_padding = len(y) % 4
    if missing_padding:
        y += '='* (4 - missing_padding)

    aliceSecretKey = base64.urlsafe_b64decode(d).hex()
    alicePublicKey = (base64.urlsafe_b64decode(x) + base64.urlsafe_b64decode(y)).hex()
    ...
    bobSecretKey = base64.urlsafe_b64decode(d).hex()
    bobPublicKey = (base64.urlsafe_b64decode(x) + base64.urlsafe_b64decode(y)).hex()
    ...
    aliceEphemeralSecretKey = base64.urlsafe_b64decode(d).hex()
    aliceEphemeralPublicKey = (base64.urlsafe_b64decode(x) + base64.urlsafe_b64decode(y)).hex()
    ...
    expected_result = unhexlify("6caf13723d14850ad4b42cd6dde935bffd2fff00a9ba70de05c203a5e1722ca7")

    assert aliceKey == bobKey, "Both parties should generate the same key using ECDH-1PU"
    assert aliceKey == expected_result, "Generated key should match the appendix A example"
