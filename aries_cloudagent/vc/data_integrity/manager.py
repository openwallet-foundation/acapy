...
class DataIntegrityManager:
    ...
    async def add_proof(self, document, options):
        ...
        secured_document["proof"].append(
            await suite.create_proof(document, options)
        )
    ...
    async def verify_proof(self, secured_document):
        ...
        verification_results['verified'] = True if all(
            result['verified'] for result in verification_results['results']
        ) else False
        return verification_results
    
    def assert_proof(self, proof):
        assert proof["cryptosuite"] in CRYPTOSUITES, \
