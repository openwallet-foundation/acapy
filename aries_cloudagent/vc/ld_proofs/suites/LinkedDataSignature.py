...
class LinkedDataSignature(LinkedDataProof, metaclass=ABCMeta):
  ...
  async def create_proof(
    ...
    if self.proof:
      ...
      proof = jsonld.compact(
          self.proof, SECURITY_CONTEXT_V2_URL,
  ...
  async def canonize_proof(
      self, proof: dict, *, document_loader: callable = None):
    print(proof)
    proof = proof.copy()
