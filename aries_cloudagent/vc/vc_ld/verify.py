...
def verify_presentation(credential: dict, suite: Any, *, purpose: Any = None):
...
# todo: controller required?
def verify_credential(
    credential: dict,
    suite: Any,
    *,
    purpose: Any = None,
    check_status: function = None,
    controller: Any = None
):
    # common credential checks
    check_credential(credential)
    ...
    if credential["credentialStatus"] and not check_status:
        raise Exception(
            'A "check_status" function must be given to verify credentials with "credentialStatus"'
        )
    ...
    # todo: set purpose to CredentialIssuancePurpose if not present
    # with controller as param

    # todo: verify credential, dependent on ld-proofs functionality
    return credential