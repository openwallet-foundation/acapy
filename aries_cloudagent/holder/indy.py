    ...
    async def store_credential(
        ...
        Args:
            ...
            credential_attr_metadata: dict mapping attribute name to (optional)
                encoding and MIME type to store as non-secret record, if specified
        ...
        if credential_attr_metadata:
            ...
            for attr in credential_data["values"]:
                meta = credential_attr_metadata.get(attr)
                tag = {
                    key: meta.get(
                        key
                    ) for key in ('encoding', 'mime-type') if meta.get(key)
                }

                if tag:  # only include non-trivial tag per attr
