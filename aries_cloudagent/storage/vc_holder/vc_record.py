...
class VCRecordSchema(BaseModelSchema):
    ...
    contexts = fields.List(fields.Str(validate=ENDPOINT_VALIDATE, metadata={
        'description': 'Context', 'example': ENDPOINT_EXAMPLE}))
    expanded_types = fields.List(fields.Str(metadata={'description':
        'JSON-LD expanded type extracted from type and context', 'example':
        'https://w3id.org/citizenship#PermanentResidentCard'}))
    schema_ids = fields.List(fields.Str(metadata={'description':
        'Schema identifier', 'example':
        'https://example.org/examples/degree.json'}))
    issuer_id = fields.Str(metadata={'description': 'Issuer identifier',
    ...
    cred_value = fields.Dict(metadata={'description':
        '(JSON-serializable) credential value'})
    given_id = fields.Str(metadata={'description': 'Credential identifier',
        'example': 'http://example.edu/credentials/3732'})
    cred_tags = fields.Dict(keys=fields.Str(metadata={'description':
        'Retrieval tag name'}), values=fields.Str(metadata={'description':
        'Retrieval tag value'}))
    record_id = fields.Str(metadata={'description': 'Record identifier',
